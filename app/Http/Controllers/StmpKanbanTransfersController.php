...
class StmpKanbanTransfersController extends Controller
{
    public function index()
    {
        $title = 'Tag Kanban TRANSFERS';
        $line_stmps          = LineStmp::all();
        $data_part_names     = DataPartName::all();
        $rm_materials        = RmMaterial::all();
        $master              = StmpTagKanbanTransfers::all();
       ...
       $data_fg_stampings = PlanningLineB3::whereBetween('created_at', [$startTime, $endTime])
           ->where('mesin', 'TRANSFERS')
           ->get();

        return view('stamping.tagkanbantranfers', compact('title','line_stmps','data_part_names','rm_materials','master','data_fg_stampings'));
    }

    public function list()
    {
        $query = DB::table('stmp_tag_kanban_transfers as a')
    ...
    public function getDropdownData()
    {
        ...
        $data = DB::table('planning_line_b3_s')
            ...
            ->where('mesin', 'TRANSFERS') // Filter mesin
...
public function store(Request $request)
{
    try {
        ...
        // Buat instance baru
        $plan = new StmpTagKanbanTransfers();
    ...
    {
        $cek = StmpTagKanbanTransfers::where('id', $request->id)->count();
        if($cek > 0){
            $row = StmpTagKanbanTransfers::where('id', $request->id)->first();
            return response()->json([
                'success'        => true,
                'id'             => $row->id,
    ...
    public function update(Request $request)
    {
        ...
        $query = StmpTagKanbanTransfers::where('id', $request->id)->update($data);
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Edit success.'
    ...
    {
        $query = StmpTagKanbanTransfers::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $query = StmpTagKanbanTransfers::where('date_plan', $request->date_plan)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = StmpTagKanbanTransfers::where('date_plan', $request->date_plan)->count();
        if($cek > 0){
            $data['sts'] = 1;
            $query = StmpTagKanbanTransfers::where('date_plan', $request->date_plan)->update($data);
            if($query){
                return response()->json([
                    'success'   => true,
                    'msg'       => 'Submit success.'
    ...
    public function cetak($id)
    {
        ...
        $master = StmpTagKanbanTransfers::findOrFail($id);
  ...
  $data_to_encode = "{$detail->part_no2}.{$detail->job_no}.{$detail->qty_ok}.{$detail->id}.{$detail->uniqNo}.{$detail->model}.{$detail->part_no}.{$detail->date_plan}.{$detail->kode_material}.{$detail->qty_ng}";
        ...
        $pdf = PDF::loadView('formatkanban.kanbanstmptransfers', $data)->setPaper($customPaper, 'potrait');
