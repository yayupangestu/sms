...
public function store(Request $request)
{
    ...
    $mesinStamping = ['LINE A1', 'LINE A2', 'LINE B1', 'LINE B2', 'LINE B3', 'LINE C1', 'LINE C2','TRANSFERS'];
    $mesinBlank = ['KOMATSU', 'FUKUI', 'AMINO'];
    ...
    public function getJobDetails(Request $request)
    {
        ...
        $jobNos = explode('/', $jobNo); // Split by slash
        $date = $request->date;
        $nextDay = date('Y-m-d', strtotime($date . ' +1 day'));

        $q1 = DB::table('upload_rekap_orders')
            ->select('uniqNo', 'qty_order', 'cycle', 'qty_kanban', 'jml_kanban', 'cycle_arrival', DB::raw("'TMMIN' as source"))
            ->whereIn('uniqNo', $jobNos)
            ->whereIn('cycle_arrival', [$date, $nextDay]);

        $q2 = DB::table('upload_rekap_order_adms')
            ->select('uniqNo', 'qty_order', 'cycle', 'qty_kanban', 'jml_kanban', 'cycle_arrival', DB::raw("'KAP' as source"))
            ->whereIn('uniqNo', $jobNos)
            ->whereIn('cycle_arrival', [$date, $nextDay]);

        $q3 = DB::table('upload_rekap_order_adm_p4_s')
            ->select('uniqNo', 'qty_order', 'cycle', 'qty_kanban', 'jml_kanban', 'cycle_arrival', DB::raw("'SAP' as source"))
            ->whereIn('uniqNo', $jobNos)
            ->whereIn('cycle_arrival', [$date, $nextDay]);

        $unionQuery = $q1->unionAll($q2)->unionAll($q3);
        ...
        $data = DB::query()
            ...
            ->select(
                ...
                DB::raw('MAX(qty_kanban) as qty_kanban'),
                DB::raw('SUM(jml_kanban) as jml_kanban'),
                DB::raw("GROUP_CONCAT(DISTINCT source SEPARATOR ', ') as source")
            )
            ->groupBy('uniqNo', 'cycle', 'cycle_arrival')
            ->get();
