...
use App\Models\StrOut20;
use App\Models\StrCategory;
use App\Models\StrUom;
use App\Models\User;
use Illuminate\Http\Request;
use App\Exports\ItemsOut20Export;
...
class StrOut20Controller extends Controller
{
    public function index()
    {
        $title = 'Item Out Store Room tm assy ';
        ...
        return view('store.out20', compact('title', 'departements', 'str_uoms', 'master_list_strs', 'users'));
    }

    public function list()
    {
        $query = DB::table('str_out20s as a')
    ...
    public function update(Request $request)
    {
        if ($request->button == "update") {
            ...
            for ($i = 1; $i < $jml + 1; $i++) {
                ...
                StrOut20::where('id', $request->idline[$i])->where('id', $request->idket[$i])->update($data);
            }
    ...
    {
        $cek = StrOut20::select(DB::raw('COUNT(doc_no) as jml'))->whereMonth('created_at', date('m'))->groupBy('doc_no')->count();
        if ($cek > 0) {
            $array = array();
            $rows = StrOut20::select('doc_no')->whereMonth('created_at', date('m'))->groupBy('doc_no')->get();
    ...
    public function update2(Request $request)
    {
        ...
        $query = StrOut20::where('id', $request->id)->update($data);
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsOut20Export($startDate, $endDate), 'Penggunaan Barang ToolMaking.xlsx');
    }

    public function destroyline(Request $request)
    {
        $query = StrOut20::where('id', $request->id)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    {
        $cek = StrOut20::where('id', $request->id)->count();
        if ($cek > 0) {
            $row = StrOut20::where('id', $request->id)->first();
            return response()->json([
                'success' => true,
                'id' => $row->id,
                'item_id' => $row->item_id,
    ...
    {
        $query = StrOut20::where('doc_no', $request->doc_no)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    {
        $cek = StrOut20::where('doc_no', $request->doc_no)->count();
        if ($cek > 0) {
            $data['sts'] = 1;
            $query = StrOut20::where('doc_no', $request->doc_no)->update($data);
            if ($query) {
                return response()->json([
                    'success' => true,
                    'msg' => 'Submit success.'
    ...
    {
        $row = StrOut20::select('date_plan', 'line_id')
    ...
    public function approve(Request $request)
    {
        ...
        try {
            ...
            $out20Items = StrOut20::where('doc_no', $doc_no)->get();
            if ($out20Items->isEmpty()) {
                return response()->json(['success' => false, 'message' => 'Records not found']);
            }

            foreach ($out20Items as $out20) {
                $item_id = $out20->item_id;
                $qty_request = $out20->qty_request;
            ...
            $firstOut20 = StrOut20::where('doc_no', $doc_no)->first();
            if ($firstOut20) {
                $firstOut20->status = 1; // Example status update
                $firstOut20->save();
            }
    ...
    {
        $doc_no = $request->input('doc_no');
        $StrOut20 = StrOut20::where('doc_no', $doc_no)->first();
        if ($StrOut20) {
            $StrOut20->status = 2;
            $StrOut20->save();
    ...
    public function saveChecked(Request $request)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $itemId) {
                ...
                $item = StrOut20::find($itemId); // Ganti dengan model yang sesuai
                if ($item) {
                    // Contoh update field
            ...
            if ($updatedDocNo) {
                $itemToUpdate = StrOut20::where('doc_no', $updatedDocNo)->first();
                if ($itemToUpdate) {
