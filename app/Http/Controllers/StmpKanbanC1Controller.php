...
class StmpKanbanC1Controller extends Controller
{
    public function index()
    {
       ...
       $data_fg_stampings = PlanningLineB3::whereBetween('created_at', [$startTime, $endTime])
        ...
        return view('stamping.tagkanbanc1', compact('title','line_stmps','data_part_names','rm_materials','master','data_fg_stampings'));
    }

    public function list()
    {
        $query = DB::table('stmp_tag_kanban_c1_s as a')
    ...
    public function getDropdownData()
    {
        ...
        $data = DB::table('planning_line_b3_s')
            ...
            ->where('mesin', 'LINE C1') // Filter mesin
...
public function store(Request $request)
{
    ...
    {
        $cek = StmpTagKanbanC1::where('id', $request->id)->count();
        if($cek > 0){
            $row = StmpTagKanbanC1::where('id', $request->id)->first();
            return response()->json([
                'success'        => true,
                'id'             => $row->id,
    ...
    public function update(Request $request)
    {
        ...
        $query = StmpTagKanbanC1::where('id', $request->id)->update($data);
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Edit success.'
    ...
    {
        $query = StmpTagKanbanC1::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $query = StmpTagKanbanC1::where('date_plan', $request->date_plan)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    // {
    //     $cek = StmpTagKanbanC1::select(DB::raw('COUNT(doc_no) as jml'))->whereMonth('created_at', date('m'))->groupBy('doc_no')->count();
    //     if($cek > 0){
    //         $array = array();
    //         $rows = StmpTagKanbanC1::select('doc_no')->whereMonth('created_at', date('m'))->groupBy('doc_no')->get();
    ...
    {
        $cek = StmpTagKanbanC1::where('date_plan', $request->date_plan)->count();
        if($cek > 0){
            $data['sts'] = 1;
            $query = StmpTagKanbanC1::where('date_plan', $request->date_plan)->update($data);
            if($query){
                return response()->json([
                    'success'   => true,
                    'msg'       => 'Submit success.'
    ...
    //     $master = StmpTagKanbanC1::findOrFail($id);
    ...
    //     $pdf = PDF::loadView('formatkanban.kanbanstmpc1', $data)->setPaper($customPaper, 'potrait');
