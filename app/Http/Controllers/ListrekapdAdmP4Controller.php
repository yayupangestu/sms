...
use App\Imports\UploadRekapP4Import; // Pastikan namespace ini benar



class ListrekapdAdmP4Controller extends Controller
{
    public function index () {
        $title = 'List Rekap';
        return view('ppic.preparedadmp4', compact('title'));
    }
    ...
    public function list()
    {
        $query = DB::table('upload_rekap_order_adm_p4_s as a')
            ->select(
                'a.created_at',
                DB::raw("GROUP_CONCAT(DISTINCT a.del_date SEPARATOR ' / ') AS del_date_group"),
                DB::raw('COUNT(*) AS total_data'),
                DB::raw('SUM(CASE WHEN a.qty_order = 0 THEN 1 ELSE 0 END) AS total_qty2'),
                DB::raw('SUM(CASE WHEN a.qty_order > 0 THEN 1 ELSE 0 END) AS total_qty_gt0')
            )
    ...
    public function checkStsAdm(Request $request)
    {
        ...
        $order = \DB::table('upload_rekap_order_adm_p4_s')
...
public function checkPcStore(Request $request)
{
    ...
    public function getQtyRekap(Request $request)
    {
        ...
        $counts = DB::table('upload_rekap_order_adm_p4_s')
            ->select(
                'cycle',
    ...
    public function listdetail(Request $request)
    {
        ...
        $query = DB::table('upload_rekap_order_adm_p4_s')
            ->select('id', 'job_no', 'part_no', 'cycle', 'jml_kanban', 'qty_kanban', 'qty_order', 'uniqNo', 'manifest', 'sts', 'del_date');
    
        if ($createdAt) {
            $query->whereDate('created_at', $createdAt); // ✅ hanya tanggal
        }
    ...
    public function updateQtyOrder(Request $request)
    {
        ...
        \DB::table('upload_rekap_order_adm_p4_s')
            ->where('job_no', $jobNo)
            ->where('manifest', $manifestExtract)
            ->update([
        ...
        \DB::table('pc_store_directs')
         ...
         // Update tabel tabel_stok_sbcs
         \DB::table('tabel_stok_sbcs')
         ->where('job_no', $jobNo)
         ->update([
             'qty_act_prepare'  => \DB::raw("GREATEST(qty_act_prepare - $qtyToSubtract, 0)"),
         ]);
