...
class DashboardPlanningBlankController extends Controller
{
    public function index()
{
    ...
    return view('blank.taglabel', compact(
...
public function cancelProduction(Request $request)
{
    $request->validate([
        ...
        // 'status_proses2' => 'required|integer',
    ]);
    ...
    // Deteksi jika Proses Manual (karena hanya saat itu qty_out_material = 1)
    if ($request->has('qty_out_material') && $request->qty_out_material !== null) {
        $dataToUpdate['qty_out_material'] = $request->qty_out_material;
        $dataToUpdate['time_startProses'] = now(); // ⬅ Tambahkan waktu akhir proses
    }

    $updated = PlanningLineB3::where('part_no', $request->part_no)
