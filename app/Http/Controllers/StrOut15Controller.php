...
use App\Models\StrOut15;
use App\Models\StrCategory;
use App\Models\StrUom;
use App\Models\User;
use Illuminate\Http\Request;
use App\Exports\ItemsOut15Export;
...
class StrOut15Controller extends Controller
{
    public function index()
    {
        ...
        return view('store.out15', compact('title', 'departements', 'str_uoms', 'master_list_strs', 'users'));
    }

    public function list()
    {
        $query = DB::table('str_out15s as a')
    ...
    public function update(Request $request)
    {
        if ($request->button === "update") {
            ...
            for ($i = 1; $i <= $jml; $i++) {
                ...
                StrOut15::where('id', $idline[$i])->update($data);
            }
    ...
    {
        $cek = StrOut15::select(DB::raw('COUNT(doc_no) as jml'))->whereMonth('created_at', date('m'))->groupBy('doc_no')->count();
        if ($cek > 0) {
            $array = array();
            $rows = StrOut15::select('doc_no')->whereMonth('created_at', date('m'))->groupBy('doc_no')->get();
    ...
    public function update2(Request $request)
    {
        ...
        $query = StrOut15::where('id', $request->id)->update($data);
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsOut15Export($startDate, $endDate), 'Penggunaan Barang Fuel Tank.xlsx');
    }

    public function destroyline(Request $request)
    {
        $query = StrOut15::where('id', $request->id)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    {
        $cek = StrOut15::where('id', $request->id)->count();
        if ($cek > 0) {
            $row = StrOut15::where('id', $request->id)->first();
            return response()->json([
                'success' => true,
                'id' => $row->id,
                'item_id' => $row->item_id,
    ...
    {
        $query = StrOut15::where('doc_no', $request->doc_no)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    {
        $cek = StrOut15::where('doc_no', $request->doc_no)->count();
        if ($cek > 0) {
            $data['sts'] = 1;
            $query = StrOut15::where('doc_no', $request->doc_no)->update($data);
            if ($query) {
                return response()->json([
                    'success' => true,
                    'msg' => 'Submit success.'
    ...
    {
        $row = StrOut15::select('date_plan', 'line_id')
    ...
    public function approve(Request $request)
    {
        ...
        try {
            ...
            $out15Items = StrOut15::where('doc_no', $doc_no)->get();
            if ($out15Items->isEmpty()) {
                return response()->json(['success' => false, 'message' => 'Records not found']);
            }

            foreach ($out15Items as $out15) {
                $item_id = $out15->item_id;
                $qty_request = $out15->qty_request;
            ...
            $firstOut15 = StrOut15::where('doc_no', $doc_no)->first();
            if ($firstOut15) {
                $firstOut15->status = 1; // Example status update
                $firstOut15->save();
            }
    ...
    {
        $doc_no = $request->input('doc_no');
        $strOut15 = StrOut15::where('doc_no', $doc_no)->first();
        if ($strOut15) {
            $strOut15->status = 2;
            $strOut15->save();
    ...
    public function saveChecked(Request $request)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $itemId) {
                ...
                $item = StrOut15::find($itemId); // Ganti dengan model yang sesuai
                if ($item) {
                    // Contoh update field
            ...
            if ($updatedDocNo) {
                $itemToUpdate = StrOut15::where('doc_no', $updatedDocNo)->first();
                if ($itemToUpdate) {
