...
class StmpKanbanC2Controller extends Controller
{
    public function index()
{
   ...
   $data_fg_stampings = PlanningLineB3::whereBetween('created_at', [$startTime, $endTime])
    ...
    return view('stamping.tagkanbanc2', compact('title','line_stmps','data_part_names','rm_materials','master','data_fg_stampings'));
}
    ...
    public function getUniqNosByMachine(Request $request)
    {
        ...
        // Ambil data rm_uniqNo dan rm_uniqNo2 berdasarkan mesin (LINE C2) dan urutkan berdasarkan created_at
        $uniqNos = DB::table('planning_line_b3_s')
        ->where('mesin', $request->input('mesin'))  // Filter berdasarkan mesin (LINE C2)
    ...
    public function getDropdownData()
    {
        ...
        $data = DB::table('planning_line_b3_s')
            ...
            ->where('mesin', 'LINE C2') // Filter mesin
...
//     // Create new entry in stmp_tag_kanban_c2_s
//     $plan               = new StmpTagKanbanC2;
...
public function store(Request $request)
{
    ...
    {
        $cek = StmpTagKanbanC2::where('id', $request->id)->count();
        if($cek > 0){
            $row = StmpTagKanbanC2::where('id', $request->id)->first();
            return response()->json([
                'success'        => true,
                'id'             => $row->id,
    ...
    public function update(Request $request)
    {
        ...
        $query = StmpTagKanbanC2::where('id', $request->id)->update($data);
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Edit success.'
    ...
    {
        $query = StmpTagKanbanC2::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $query = StmpTagKanbanC2::where('date_plan', $request->date_plan)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    // {
    //     $cek = StmpTagKanbanC2::select(DB::raw('COUNT(doc_no) as jml'))->whereMonth('created_at', date('m'))->groupBy('doc_no')->count();
    //     if($cek > 0){
    //         $array = array();
    //         $rows = StmpTagKanbanC2::select('doc_no')->whereMonth('created_at', date('m'))->groupBy('doc_no')->get();
    ...
    {
        $cek = StmpTagKanbanC2::where('date_plan', $request->date_plan)->count();
        if($cek > 0){
            $data['sts'] = 1;
            $query = StmpTagKanbanC2::where('date_plan', $request->date_plan)->update($data);
            if($query){
                return response()->json([
                    'success'   => true,
                    'msg'       => 'Submit success.'
    ...
    //     $master = StmpTagKanbanC2::findOrFail($id);
    ...
    //     $query = DB::table('stmp_tag_kanban_c2_s as a')
    //             ->select('a.id', 'a.part_name','a.part_no','a.part_no','a.part_no2','a.job_no','a.model','a.name_material', 'a.qty_ok', 'a.qty_ng','a.model', 'a.time_start', 'a.time_end', 'a.shift', 'a.uniqNo','g.username as createdby','a.shift','a.date_plan','a.tujuan','b.name as line_id','a.line','a.kode_material','a.part_no_rm')
    //             ->join('line_stmps as b', 'b.id', '=', 'a.line_id', 'left')
    ...
    //     $pdf = PDF::loadView('formatkanban.kanbanstmpc2', $data)->setPaper($customPaper, 'potrait');
