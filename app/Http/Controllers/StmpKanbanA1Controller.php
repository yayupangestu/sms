...
class StmpKanbanA1Controller extends Controller
{
    public function index()
    {
       ...
       $data_fg_stampings = PlanningLineB3::whereBetween('created_at', [$startTime, $endTime])
        ...
        return view('stamping.tagkanbana1', compact('title','line_stmps','data_part_names','rm_materials','master','data_fg_stampings'));
    }

    public function list()
    {
        $query = DB::table('stmp_tag_kanban_a1_s as a')
    ...
    public function getDropdownData()
    {
        ...
        $data = DB::table('planning_line_b3_s')
            ...
            ->where('mesin', 'LINE A1') // Filter mesin
...
public function store(Request $request)
{
    ...
    {
        $cek = StmpTagKanbanA1::where('id', $request->id)->count();
        if($cek > 0){
            $row = StmpTagKanbanA1::where('id', $request->id)->first();
            return response()->json([
                'success'        => true,
                'id'             => $row->id,
    ...
    public function update(Request $request)
    {
        ...
        $query = StmpTagKanbanA1::where('id', $request->id)->update($data);
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Edit success.'
    ...
    {
        $query = StmpTagKanbanA1::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $query = StmpTagKanbanA1::where('date_plan', $request->date_plan)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = StmpTagKanbanA1::where('date_plan', $request->date_plan)->count();
        if($cek > 0){
            $data['sts'] = 1;
            $query = StmpTagKanbanA1::where('date_plan', $request->date_plan)->update($data);
            if($query){
                return response()->json([
                    'success'   => true,
                    'msg'       => 'Submit success.'
    ...
    public function cetak($id)
    {
        ...
        $master = StmpTagKanbanA1::findOrFail($id);
       ...
       $data_to_encode = "{$detail->part_no2}.{$detail->job_no}.{$detail->qty_ok}.{$detail->id}.{$detail->uniqNo}.{$detail->model}.{$detail->part_no}.{$detail->date_plan}.{$detail->kode_material}.{$detail->qty_ng}";
        ...
        $pdf = PDF::loadView('formatkanban.kanbanstmpa1', $data)->setPaper($customPaper, 'potrait');
