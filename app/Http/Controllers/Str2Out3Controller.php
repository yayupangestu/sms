...
use App\Models\Str2Out3;
use App\Models\StrCategory;
use App\Models\StrUom;
use App\Models\User;
use App\Exports\ItemsOut3ExportStr2;
...
class Str2Out3Controller extends Controller
{
    public function index()
    {
        ...
        return view('store2.out3', compact('title', 'departements', 'str_uoms', 'master_list_strs', 'users'));
    }

    public function list()
    {
        $query = DB::table('str2_out3s as a')
    ...
    public function update(Request $request)
    {
        if ($request->button == "update") {
            ...
            for ($i = 1; $i < $jml + 1; $i++) {
                ...
                Str2Out3::where('id', $request->idline[$i])->where('id', $request->idket[$i])->update($data);
            }
    ...
    {
        $cek = Str2Out3::select(DB::raw('COUNT(doc_no) as jml'))->whereMonth('created_at', date('m'))->groupBy('doc_no')->count();
        if ($cek > 0) {
            $array = array();
            $rows = Str2Out3::select('doc_no')->whereMonth('created_at', date('m'))->groupBy('doc_no')->get();
    ...
    public function update2(Request $request)
    {
        ...
        $query = Str2Out3::where('id', $request->id)->update($data);
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsOut3ExportStr2($startDate, $endDate), 'Penggunaan Barang LINE A ASI-2.xlsx');
    }

    public function destroyline(Request $request)
    {
        $query = Str2Out3::where('id', $request->id)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    {
        $cek = Str2Out3::where('id', $request->id)->count();
        if ($cek > 0) {
            $row = Str2Out3::where('id', $request->id)->first();
            return response()->json([
                'success' => true,
                'id' => $row->id,
                'item_id' => $row->item_id,
    ...
    {
        $query = Str2Out3::where('doc_no', $request->doc_no)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    {
        $cek = Str2Out3::where('doc_no', $request->doc_no)->count();
        if ($cek > 0) {
            $data['sts'] = 1;
            $query = Str2Out3::where('doc_no', $request->doc_no)->update($data);
            if ($query) {
                return response()->json([
                    'success' => true,
                    'msg' => 'Submit success.'
    ...
    {
        $row = Str2Out3::select('date_plan', 'line_id')
    ...
    public function approve(Request $request)
    {
        ...
        try {
            ...
            $out3Items = Str2Out3::where('doc_no', $doc_no)->get();
            ...
            $firstOut3 = Str2Out3::where('doc_no', $doc_no)->first();
            if ($firstOut3) {
    ...
    {
        $doc_no = $request->input('doc_no');
        $strOut3 = Str2Out3::where('doc_no', $doc_no)->first();
        if ($strOut3) {
            $strOut3->status = 2;
            $strOut3->save();
    ...
    public function saveChecked(Request $request)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $itemId) {
                ...
                $item = Str2Out3::find($itemId); // Ganti dengan model yang sesuai
                if ($item) {
                    // Contoh update field
            ...
            if ($updatedDocNo) {
                $itemToUpdate = Str2Out3::where('doc_no', $updatedDocNo)->first();
                if ($itemToUpdate) {
