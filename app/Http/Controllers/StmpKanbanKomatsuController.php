...
use App\Models\StmpTagKanbanKomatsu;
...
class StmpKanbanKomatsuController extends Controller
{
    public function index()
    {
        ...
        $master              = StmpTagKanbanKomatsu::all();
       ...
       $data_fg_stampings = PlanningLineB3::whereBetween('created_at', [$startTime, $endTime])
        ...
        return view('stamping.tagkanbankomatsu', compact('title','line_stmps','data_part_names','rm_materials','master','data_fg_stampings'));
    }

    public function list()
    {
        $query = DB::table('stmp_tag_kanban_komatsus as a')
    ...
    public function getUniqNosByMachine(Request $request)
    {
        ...
        ->where('mesin', $request->input('mesin')) 
        ->orderBy('created_at', 'desc') 
        ->get(['rm_uniqNo', 'rm_uniqNo2']); 
    ...
    public function getDropdownData()
    {
        ...
        $data = DB::table('planning_line_b3_s')
            ...
            ->where('mesin', 'KOMATSU') // Filter mesin
...
public function store(Request $request)
{
    try {
        ...
        if (!$request->uniqNo) {
            ...
            $plan->uniqNo = 'B' . date('dis', strtotime($currentTimestamp)); // Format: ddmmyyHHmmss
        } else {
    ...
    {
        $cek = StmpTagKanbanKomatsu::where('id', $request->id)->count();
        if($cek > 0){
            $row = StmpTagKanbanKomatsu::where('id', $request->id)->first();
            return response()->json([
                'success'        => true,
                'id'             => $row->id,
    ...
    public function update(Request $request)
    {
        ...
        $query = StmpTagKanbanKomatsu::where('id', $request->id)->update($data);
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Edit success.'
    ...
    {
        $query = StmpTagKanbanKomatsu::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $query = StmpTagKanbanKomatsu::where('date_plan', $request->date_plan)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = StmpTagKanbanKomatsu::where('date_plan', $request->date_plan)->count();
        if($cek > 0){
            $data['sts'] = 1;
            $query = StmpTagKanbanKomatsu::where('date_plan', $request->date_plan)->update($data);
            if($query){
                return response()->json([
                    'success'   => true,
                    'msg'       => 'Submit success.'
    ...
    {
        $master = StmpTagKanbanKomatsu::findOrFail($id);
        $data_to_encode = 
        ($master->part_no ?? '0') . '||' .
        ($master->kode_material ?? '0') . '||' .
        ($master->line_stmp ?? '0') . '||' .
        ($master->uniqNo ?? '0') . '||' .
        ($master->part_no2 ?? '0') . '||' .
        ($master->id ?? '0') . '||' .
        ...
        $data = [
            'qrcode'            => $qrcode,
            // 'uniqNo'         => $uniqNo,
            'uniqNo'            => $master->uniqNo,
            'part_no'           => $master->part_no,
            'line_stmp'         => $master->line_stmp,
            'part_name'         => $master->part_name,
            'createdby'         => $master->createdby,
            'qty_ok'           => $master->qty_ok,
            'job_no'            => $master->job_no,  // Format dynamically without rounding
            'model'          => $master->model,
            'kode_material'     => $master->kode_material,
            // 'kg_sheet'       => $master->kg_sheet,
            'created_at'        => $master->created_at,
            // 'supplier'       => $master->supplier,
            // 'actual'         => $master->actual,
            'spec'              => $master->spec,
            'no_rak'            => $master->no_rak,
            // 'uniq_no'        => $master->uniq_no,
        ];
        ...
        return $pdf->stream(date('d_M_Y') . '_' . $master->id . '_qrcode.pdf');
    }
