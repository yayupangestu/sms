...
class UploadRekapController extends Controller
{
    public function index(){
        $title = 'Rekap Order';
        return view('ppic.uploadrekap', compact('title'));
    ...
    public function list()
    {
        ...
        return DataTables::of($query)
            ...
            ->addColumn('arrival3', function($row) {
                if (!$row->arrivals) return '-';
                $arrivals = array_filter(explode('|', $row->arrivals));
                $arrivals = array_values($arrivals);
                return $arrivals[2] ?? '-';
    ...
    {
        try {
            $affected = DB::table('upload_rekap_orders')
    ...
    {
        $query = UploadRekapOrder::where('created_at', $request->created_at)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    public function importRekap(Request $request)
{
    ...
    try {
        ...
        Excel::import(new UploadRekapImport($path), $file);
    ...
    public function exportPdf(Request $request)
    {
        ...
        $data = DB::table('upload_rekap_orders as a')
            ->select(
                'a.manifest',
                'a.part_name',
                'a.part_no',
        ...
        $pdf = Pdf::loadView('ppic.pdf_rekap_standard', compact('data', 'created_at'))
                  ->setPaper('a4', 'landscape');

        $filename = 'Rekap_Order_Standard_' . str_replace([':', ' '], ['-', '_'], $created_at) . '.pdf';
    ...
    public function getSummary(Request $request)
    {
        ...
        $summary = DB::table('upload_rekap_orders')
    ...
    public function getChartData(Request $request)
    {
        ...
        $totalWorks = DB::table('upload_rekap_orders');
        if ($cycleArrivalDate && !empty($cycleArrivalDate)) {
        ...
        $approvedWorks = DB::table('upload_rekap_orders')
            ->where('qty_order', 0)
        ...
        $notScannedWorks = DB::table('upload_rekap_orders')
            ->where('qty_order', 0)
            ->where(function($q) {
        ...
        $team = ['Fikri', 'Ajun', 'Abi', 'Emif'];
        $teamCounts = [];
        foreach ($team as $member) {
            $mQuery = DB::table('upload_rekap_orders')
