...
class PlanningLineC1Controller extends Controller
{
    public function index()
    {
        ...
        $tabel_c1_s = DB::table('tabel_c1_s as a')
                ...
                ->get();
        return view('planningline.planningc1', compact('title','line_stmps','data_part_names','rm_materials','tabel_c1_s'));
    }
    ...
    {
        $cek = PlanningLineC1::where('id', $request->id)->count();
        if($cek > 0){
            $row = PlanningLineC1::where('id', $request->id)->first();
            return response()->json([
                'success'        => true,
                'id'             => $row->id,
    ...
    public function update(Request $request)
    {
        ...
        $query = PlanningLineC1::where('id', $request->id)->update($data);
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Edit success.'
    ...
    {
        $query = PlanningLineC1::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $query = PlanningLineC1::where('date_plan', $request->date_plan)->where('line_id', $request->idline)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
