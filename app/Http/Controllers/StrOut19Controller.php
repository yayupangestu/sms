...
use App\Models\StrOut19;
use App\Models\StrCategory;
use App\Models\StrUom;
use App\Models\User;
use Illuminate\Http\Request;
use App\Exports\ItemsOut19Export;
...
class StrOut19Controller extends Controller
{
    public function index()
    {
        ...
        return view('store.out19', compact('title', 'departements', 'str_uoms', 'master_list_strs', 'users'));
    }

    public function list()
    {
        $query = DB::table('str_out19s as a')
    ...
    public function update(Request $request)
    {
        if ($request->button == "update") {
            ...
            for ($i = 1; $i < $jml + 1; $i++) {
                ...
                StrOut19::where('id', $request->idline[$i])->where('id', $request->idket[$i])->update($data);
            }
    ...
    {
        $cek = StrOut19::select(DB::raw('COUNT(doc_no) as jml'))->whereMonth('created_at', date('m'))->groupBy('doc_no')->count();
        if ($cek > 0) {
            $array = array();
            $rows = StrOut19::select('doc_no')->whereMonth('created_at', date('m'))->groupBy('doc_no')->get();
    ...
    public function update2(Request $request)
    {
        ...
        $query = StrOut19::where('id', $request->id)->update($data);
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsOut19Export($startDate, $endDate), 'Penggunaan Barang MIP.xlsx');
    }

    public function destroyline(Request $request)
    {
        $query = StrOut19::where('id', $request->id)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    {
        $cek = StrOut19::where('id', $request->id)->count();
        if ($cek > 0) {
            $row = StrOut19::where('id', $request->id)->first();
            return response()->json([
                'success' => true,
                'id' => $row->id,
                'item_id' => $row->item_id,
    ...
    {
        $query = StrOut19::where('doc_no', $request->doc_no)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    {
        $cek = StrOut19::where('doc_no', $request->doc_no)->count();
        if ($cek > 0) {
            $data['sts'] = 1;
            $query = StrOut19::where('doc_no', $request->doc_no)->update($data);
            if ($query) {
                return response()->json([
                    'success' => true,
                    'msg' => 'Submit success.'
    ...
    {
        $row = StrOut19::select('date_plan', 'line_id')
    ...
    public function approve(Request $request)
    {
        ...
        try {
            ...
            $out19Items = StrOut19::where('doc_no', $doc_no)->get();
            if ($out19Items->isEmpty()) {
                return response()->json(['success' => false, 'message' => 'Records not found']);
            }

            foreach ($out19Items as $out19) {
                $item_id = $out19->item_id;
                $qty_request = $out19->qty_request;
            ...
            $firstOut19 = StrOut19::where('doc_no', $doc_no)->first();
            if ($firstOut19) {
                $firstOut19->status = 1; // Example status update
                $firstOut19->save();
            }
    ...
    {
        $doc_no = $request->input('doc_no');
        $StrOut19 = StrOut19::where('doc_no', $doc_no)->first();
        if ($StrOut19) {
            $StrOut19->status = 2;
            $StrOut19->save();
    ...
    public function saveChecked(Request $request)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $itemId) {
                ...
                $item = StrOut19::find($itemId); // Ganti dengan model yang sesuai
                if ($item) {
                    // Contoh update field
            ...
            if ($updatedDocNo) {
                $itemToUpdate = StrOut19::where('doc_no', $updatedDocNo)->first();
                if ($itemToUpdate) {
