...
use App\Models\User;
use App\Exports\ConsumExport;
use App\Exports\ItemsConsumSummaryExport;
use Maatwebsite\Excel\Facades\Excel;
use Illuminate\Http\Request;
use DataTables;
use DB;

class StrStokConsumController extends Controller
{
    public function index()
    {
        ...
        $master_list_strs = DB::table('master_list_strs as a')
            ...
            ->get();
        return view('store.stokconsum', compact('title', 'str_uoms', 'master_list_strs', 'str_uoms'));
    }


    public function list()
    {
        $query = DB::table('str_stok_consums as a')
    ...
    {
        $cek = StrStokConsum::where('id', $request->id)->count();
        if ($cek > 0) {
            $row = StrStokConsum::where('id', $request->id)->first();
            return response()->json([
                'success' => true,
                'id' => $row->id,
                'item_id' => $row->item_id,
    ...
    public function update(Request $request)
    {
        ...
        $query = StrStokConsum::where('id', $request->id)->update($data);
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Update success.'
    ...
    {
        $query = StrStokConsum::where('id', $request->id)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    public function export()
    {
        ...
        return Excel::download(new ConsumExport, 'ConsumExport.xlsx');
    }
    ...
    public function exportSummary(Request $request)
    {
        ...
        return Excel::download(new ItemsConsumSummaryExport($start, $end), 'Summary Consum Out.xlsx');
    }
}
