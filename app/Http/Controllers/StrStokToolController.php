...
use App\Models\User;
use App\Exports\ToolInsertExport;
use Maatwebsite\Excel\Facades\Excel;
use Illuminate\Http\Request;
use DataTables;
use DB;

class StrStokToolController extends Controller
{
    public function index()
    {
        $title = 'STOK TOOL & INSERT';
        $str_uoms = StrUom::all();
        ...
        $master_list_strs = DB::table('master_list_strs as a')
            ...
            ->get();
        return view('store.stoktool', compact('title', 'str_uoms', 'master_list_strs', 'str_uoms'));
    }
    ...
    {
        $query = DB::table('str_stok_tis as a')
    ...
    {
        $cek = StrStokTi::where('id', $request->id)->count();
        if ($cek > 0) {
            $row = StrStokTi::where('id', $request->id)->first();
            return response()->json([
                'success' => true,
                'id' => $row->id,
                'item_id' => $row->item_id,
    ...
    public function update(Request $request)
    {
        ...
        $query = StrStokTi::where('id', $request->id)->update($data);
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Update success.'
    ...
    {
        $query = StrStokTi::where('id', $request->id)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    {
        return Excel::download(new ToolInsertExport, 'Tool&Insert.xlsx');
    }
}
