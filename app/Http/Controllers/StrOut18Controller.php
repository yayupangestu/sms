...
use App\Exports\ItemsOut18Export;
...
class StrOut18Controller extends Controller
{
    public function index()
    {
        ...
        return view('store.out18', compact('title', 'departements', 'str_uoms', 'master_list_strs', 'users'));
    }

    public function list()
    {
        $query = DB::table('str_out18s as a')
    ...
    public function update(Request $request)
    {
        if ($request->button == "update") {
            ...
            for ($i = 1; $i < $jml + 1; $i++) {
                ...
                StrOut18::where('id', $request->idline[$i])->where('id', $request->idket[$i])->update($data);
            }
    ...
    {
        $cek = StrOut18::select(DB::raw('COUNT(doc_no) as jml'))->whereMonth('created_at', date('m'))->groupBy('doc_no')->count();
        if ($cek > 0) {
            $array = array();
            $rows = StrOut18::select('doc_no')->whereMonth('created_at', date('m'))->groupBy('doc_no')->get();
    ...
    public function update2(Request $request)
    {
        ...
        $query = StrOut18::where('id', $request->id)->update($data);
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsOut18Export($startDate, $endDate), 'Penggunaan Barang ToolMaking.xlsx');
    }

    public function destroyline(Request $request)
    {
        $query = StrOut18::where('id', $request->id)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    {
        $cek = StrOut18::where('id', $request->id)->count();
        if ($cek > 0) {
            $row = StrOut18::where('id', $request->id)->first();
            return response()->json([
                'success' => true,
                'id' => $row->id,
                'item_id' => $row->item_id,
    ...
    {
        $query = StrOut18::where('doc_no', $request->doc_no)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    {
        $cek = StrOut18::where('doc_no', $request->doc_no)->count();
        if ($cek > 0) {
            $data['sts'] = 1;
            $query = StrOut18::where('doc_no', $request->doc_no)->update($data);
            if ($query) {
                return response()->json([
                    'success' => true,
                    'msg' => 'Submit success.'
    ...
    {
        $row = StrOut18::select('date_plan', 'line_id')
    ...
    public function approve(Request $request)
    {
        ...
        try {
            ...
            $out18Items = StrOut18::where('doc_no', $doc_no)->get();
            if ($out18Items->isEmpty()) {
                return response()->json(['success' => false, 'message' => 'Records not found']);
            }

            foreach ($out18Items as $out18) {
                $item_id = $out18->item_id;
                $qty_request = $out18->qty_request;
            ...
            $firstOut18 = StrOut18::where('doc_no', $doc_no)->first();
            if ($firstOut18) {
                $firstOut18->status = 1; // Example status update
                $firstOut18->save();
            }
    ...
    {
        $doc_no = $request->input('doc_no');
        $StrOut18 = StrOut18::where('doc_no', $doc_no)->first();
        if ($StrOut18) {
            $StrOut18->status = 2;
            $StrOut18->save();
    ...
    public function saveChecked(Request $request)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $itemId) {
                ...
                $item = StrOut18::find($itemId); // Ganti dengan model yang sesuai
                if ($item) {
                    // Contoh update field
            ...
            if ($updatedDocNo) {
                $itemToUpdate = StrOut18::where('doc_no', $updatedDocNo)->first();
                if ($itemToUpdate) {
