...
class ReportC1Controller extends Controller
{
    public function index()
    {
        $title = 'Report LINE C1';
        $line_stmps = LineStmp::all();
        $data_part_names = DataPartName::all();
        $stmp_tag_kanban_c1_s = StmpTagKanban::all();
        $data_models = DataModel::all();
        $users = User::all();
        
        return view('planningline.reportc1', compact('title','line_stmps','data_part_names','users','stmp_tag_kanban_c1_s'));
    }
    ...
    public function list(Request $request)
    {
        $query = DB::table('stmp_tag_kanban_c1_s as a')
                ...
                ->join('stmp_tag_kanban_c1_s as c', 'c.id', '=', 'a.line_stmp', 'left')
