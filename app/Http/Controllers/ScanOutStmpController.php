...
use App\Models\StmpTagKanbanB2;
use App\Models\StmpTagKanbanA1;
use App\Models\StmpTagKanbanA2;
use App\Models\StmpTagKanbanTransfers;
...
class ScanOutStmpController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($StmpTagKanbanC1->isNotEmpty()) {
          ...
          $StmpTagKanbanB1 = StmpTagKanbanB1::where('uniqNo', $request->input('uniqNo'))
            // ->orWhere('part_no', $request->input('part_no_rm'))
            ->get(); // Ambil semua data yang cocok

        if ($StmpTagKanbanB1->isNotEmpty()) {
            foreach ($StmpTagKanbanB1 as $item) {
                $item->sts_time = now();
                $item->sts_user = auth()->user()->username;
                $item->sts_scan = 1;
                $item->save();
         ...
         $StmpTagKanbanB2 = StmpTagKanbanB2::where('uniqNo', $request->input('uniqNo'))
            // ->orWhere('part_no', $request->input('part_no_rm'))
            ->get(); // Ambil semua data yang cocok

        if ($StmpTagKanbanB2->isNotEmpty()) {
            foreach ($StmpTagKanbanB2 as $item) {
                $item->sts_time = now();
                $item->sts_user = auth()->user()->username;
                $item->sts_scan = 1;
                $item->save();
         ...
         $StmpTagKanbanTransfers = StmpTagKanbanTransfers::where('uniqNo', $request->input('uniqNo'))
            // ->orWhere('part_no', $request->input('part_no_rm'))
            ->get(); // Ambil semua data yang cocok

        if ($StmpTagKanbanTransfers->isNotEmpty()) {
            foreach ($StmpTagKanbanTransfers as $item) {
                $item->sts_time = now();
                $item->sts_user = auth()->user()->username;
                $item->sts_scan = 1;
                $item->save();
         ...
         $StmpTagKanbanA1 = StmpTagKanbanA1::where('uniqNo', $request->input('uniqNo'))
            // ->orWhere('part_no', $request->input('part_no_rm'))
            ->get(); // Ambil semua data yang cocok

        if ($StmpTagKanbanA1->isNotEmpty()) {
            foreach ($StmpTagKanbanA1 as $item) {
                $item->sts_time = now();
                $item->sts_user = auth()->user()->username;
                $item->sts_scan = 1;
                $item->save();
         ...
         $StmpTagKanbanA2 = StmpTagKanbanA2::where('uniqNo', $request->input('uniqNo'))
            // ->orWhere('part_no', $request->input('part_no_rm'))
            ->get(); // Ambil semua data yang cocok

        if ($StmpTagKanbanA2->isNotEmpty()) {
            foreach ($StmpTagKanbanA2 as $item) {
                $item->sts_time = now();
                $item->sts_user = auth()->user()->username;
                $item->sts_scan = 1;
                $item->save();
         ...
         if ($StmpTagKanbanC2->isNotEmpty()) {
            ...
            if ($PlanningLineB3) {
                ...
                if ($PlanningLineB3->actual_production == $PlanningLineB3->qty_plan) {
                    ...
                    $PlanningLineB3->status_proses2 = 3;
                ...
                }  else if (empty($PlanningLineB3->stmp_out_qty8) && empty($PlanningLineB3->stmp_out_partNo8) && empty($PlanningLineB3->stmp_out_jobNo8)) {
                    $PlanningLineB3->stmp_out_uniqNo8 = $request->input('uniqNo');
                    $PlanningLineB3->stmp_out_jobNo8 = $request->input('job_no');
                    $PlanningLineB3->stmp_out_partNo8 = $request->input('part_no');
                    $PlanningLineB3->stmp_out_model8 = $request->input('model');
                    $PlanningLineB3->stmp_out_qty8 = $request->input('qty');
                    $PlanningLineB3->stmp_out_qty_ng8 = $request->input('qty_ng');
                    $PlanningLineB3->stmp_out_date8 = $request->input('date');
                    $PlanningLineB3->stmp_out_kodematerial8 = $request->input('kodeMaterial');
                    // $PlanningLineB3->stmp_out_start8 = $request->input('part_no_rm');
                    $PlanningLineB3->stmp_out_end8 = $request->input('end');
                    $PlanningLineB3->stmp_out_user8 = auth()->user()->username;
                    $PlanningLineB3->stmp_out_leader_line8 = auth()->user()->line_id;
                    $PlanningLineB3->stmp_out_time8 = now();
                }  else if (empty($PlanningLineB3->stmp_out_qty9) && empty($PlanningLineB3->stmp_out_partNo9) && empty($PlanningLineB3->stmp_out_jobNo9)) {
                    $PlanningLineB3->stmp_out_uniqNo9 = $request->input('uniqNo');
                    $PlanningLineB3->stmp_out_jobNo9 = $request->input('job_no');
                    $PlanningLineB3->stmp_out_partNo9 = $request->input('part_no');
                    $PlanningLineB3->stmp_out_model9 = $request->input('model');
                    $PlanningLineB3->stmp_out_qty9 = $request->input('qty');
                    $PlanningLineB3->stmp_out_qty_ng9 = $request->input('qty_ng');
                    $PlanningLineB3->stmp_out_date9 = $request->input('date');
                    $PlanningLineB3->stmp_out_kodematerial9 = $request->input('kodeMaterial');
                    // $PlanningLineB3->stmp_out_start9 = $request->input('part_no_rm');
                    $PlanningLineB3->stmp_out_end9 = $request->input('end');
                    $PlanningLineB3->stmp_out_user9 = auth()->user()->username;
                    $PlanningLineB3->stmp_out_leader_line9 = auth()->user()->line_id;
                    $PlanningLineB3->stmp_out_time9 = now();
                }  else if (empty($PlanningLineB3->stmp_out_qty10) && empty($PlanningLineB3->stmp_out_partNo10) && empty($PlanningLineB3->stmp_out_jobNo10)) {
                    $PlanningLineB3->stmp_out_uniqNo10 = $request->input('uniqNo');
                    $PlanningLineB3->stmp_out_jobNo10 = $request->input('job_no');
                    $PlanningLineB3->stmp_out_partNo10 = $request->input('part_no');
                    $PlanningLineB3->stmp_out_model10 = $request->input('model');
                    $PlanningLineB3->stmp_out_qty10 = $request->input('qty');
                    $PlanningLineB3->stmp_out_qty_ng10 = $request->input('qty_ng');
                    $PlanningLineB3->stmp_out_date10 = $request->input('date');
                    $PlanningLineB3->stmp_out_kodematerial10 = $request->input('kodeMaterial');
                    // $PlanningLineB3->stmp_out_start10 = $request->input('part_no_rm');
                    $PlanningLineB3->stmp_out_end10 = $request->input('end');
                    $PlanningLineB3->stmp_out_user10 = auth()->user()->username;
                    $PlanningLineB3->stmp_out_leader_line10 = auth()->user()->line_id;
                    $PlanningLineB3->stmp_out_time10 = now();
                } else {
                    return response()->json([
                        'success' => false,
