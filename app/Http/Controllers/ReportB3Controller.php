...
class ReportB3Controller extends Controller
{
    public function index()
    {
        $title = 'Report Line B3';
        $line_stmps = LineStmp::all();
        $data_part_names = DataPartName::all();
        $stmp_tag_kanbans = StmpTagKanban::all();
        $data_models = DataModel::all();
        $users = User::all();
        
        return view('planningline.reportb3', compact('title','line_stmps','data_part_names','users','stmp_tag_kanbans'));
    }
    ...
    public function list(Request $request)
    {
        $query = DB::table('stmp_tag_kanbans as a')
                ...
                ->join('stmp_tag_kanbans as c', 'c.id', '=', 'a.line_stmp', 'left')
