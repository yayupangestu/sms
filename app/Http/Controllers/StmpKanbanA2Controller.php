...
class StmpKanbanA2Controller extends Controller
{
    public function index()
    {
       ...
       $data_fg_stampings = PlanningLineB3::whereBetween('created_at', [$startTime, $endTime])
        ...
        return view('stamping.tagkanbana2', compact('title','line_stmps','data_part_names','rm_materials','master','data_fg_stampings'));
    }

    public function list()
    {
        $query = DB::table('stmp_tag_kanban_a2_s as a')
    ...
    public function getUniqNosByMachine(Request $request)
    {
        ...
        // Ambil data stmp_in_uniqNo dan stmp_in_uniqNo2 berdasarkan mesin (LINE C1) dan urutkan berdasarkan created_at
        ...
        ->get(['stmp_in_uniqNo', 'stmp_in_uniqNo2']); // Ambil stmp_in_uniqNo dan stmp_in_uniqNo2
    ...
    public function getDropdownData()
    {
        ...
        $data = DB::table('planning_line_b3_s')
            ...
            ->where('mesin', 'LINE A2') // Filter mesin
            ->where('status', 2) // Hanya ambil data dengan status = 2
            ->where('status_proses', '!=', 4) // Kecualikan status_proses = 3
            ->select('part_no', 'stmp_in_uniqNo', 'stmp_in_uniqNo2', 'stmp_in_uniqNo3', 'stmp_in_uniqNo4', 'stmp_in_uniqNo5')
            ->get();
        ...
        $mergedData = collect($data)
            ->flatMap(function ($item) {
                return [
                    ['part_no' => $item->part_no, 'stmp_in_uniqNo' => $item->stmp_in_uniqNo],
                    ['part_no' => $item->part_no, 'stmp_in_uniqNo' => $item->stmp_in_uniqNo2],
                    ['part_no' => $item->part_no, 'stmp_in_uniqNo' => $item->stmp_in_uniqNo3],
                    ['part_no' => $item->part_no, 'stmp_in_uniqNo' => $item->stmp_in_uniqNo4],
                    ['part_no' => $item->part_no, 'stmp_in_uniqNo' => $item->stmp_in_uniqNo5]
                ];
            })
            ->unique()
            ->filter(function ($item) {
                return !empty($item['stmp_in_uniqNo']); // Hapus nilai null atau kosong
            })
            ->values();
...
public function getDropdownData2(Request $request)
{
    ...
    // Gabungkan uniqNo dari stmp_in_uniqNo sampai stmp_in_uniqNo5
    $uniqNos = collect($data)
        ->flatMap(function ($item) {
            return [
                $item->stmp_in_uniqNo,
                $item->stmp_in_uniqNo2,
                $item->stmp_in_uniqNo3,
                $item->stmp_in_uniqNo4,
                $item->stmp_in_uniqNo5
            ];
        })
        ->filter()
        ->unique()
...
public function store(Request $request)
{
    ...
    {
        $cek = StmpTagKanbanA2::where('id', $request->id)->count();
        if($cek > 0){
            $row = StmpTagKanbanA2::where('id', $request->id)->first();
            return response()->json([
                'success'        => true,
                'id'             => $row->id,
    ...
    public function update(Request $request)
    {
        ...
        $query = StmpTagKanbanA2::where('id', $request->id)->update($data);
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Edit success.'
    ...
    {
        $query = StmpTagKanbanA2::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $query = StmpTagKanbanA2::where('date_plan', $request->date_plan)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = StmpTagKanbanA2::where('date_plan', $request->date_plan)->count();
        if($cek > 0){
            $data['sts'] = 1;
            $query = StmpTagKanbanA2::where('date_plan', $request->date_plan)->update($data);
            if($query){
                return response()->json([
                    'success'   => true,
                    'msg'       => 'Submit success.'
    ...
    public function cetak($id)
    {
        ...
        $master = StmpTagKanbanA2::findOrFail($id);
        ...
        $pdf = PDF::loadView('formatkanban.kanbanstmpa2', $data)->setPaper($customPaper, 'potrait');
