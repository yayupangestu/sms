...
use App\Models\StrOut16;
use App\Models\StrCategory;
use App\Models\StrUom;
use App\Models\User;
use Illuminate\Http\Request;
use App\Exports\ItemsOut16Export;
...
class StrOut16Controller extends Controller
{
    public function index()
    {
        ...
        return view('store.out16', compact('title', 'departements', 'str_uoms', 'master_list_strs', 'users'));
    }

    public function list()
    {
        $query = DB::table('str_out16s as a')
    ...
    public function update(Request $request)
    {
        if ($request->button === "update") {
            ...
            for ($i = 1; $i <= $jml; $i++) {
                ...
                $data = [
                    'qty_out' => $qty[$i] ?? null,
                    'keterangan' => $ket[$i] ?? null,
                    'updateby' => auth()->user()->id
                ];

                StrOut16::where('id', $idline[$i])->update($data);
            }
    ...
    {
        $cek = StrOut16::select(DB::raw('COUNT(doc_no) as jml'))->whereMonth('created_at', date('m'))->groupBy('doc_no')->count();
        if ($cek > 0) {
            $array = array();
            $rows = StrOut16::select('doc_no')->whereMonth('created_at', date('m'))->groupBy('doc_no')->get();
    ...
    public function update2(Request $request)
    {
        ...
        $query = StrOut16::where('id', $request->id)->update($data);
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsOut16Export($startDate, $endDate), 'Penggunaan Barang Service Part.xlsx');
    }

    public function destroyline(Request $request)
    {
        $query = StrOut16::where('id', $request->id)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    {
        $cek = StrOut16::where('id', $request->id)->count();
        if ($cek > 0) {
            $row = StrOut16::where('id', $request->id)->first();
            return response()->json([
                'success' => true,
                'id' => $row->id,
                'item_id' => $row->item_id,
    ...
    {
        $query = StrOut16::where('doc_no', $request->doc_no)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    {
        $cek = StrOut16::where('doc_no', $request->doc_no)->count();
        if ($cek > 0) {
            $data['sts'] = 1;
            $query = StrOut16::where('doc_no', $request->doc_no)->update($data);
            if ($query) {
                return response()->json([
                    'success' => true,
                    'msg' => 'Submit success.'
    ...
    {
        $row = StrOut16::select('date_plan', 'line_id')
    ...
    public function approve(Request $request)
    {
        ...
        try {
            ...
            $out16Items = StrOut16::where('doc_no', $doc_no)->get();
            if ($out16Items->isEmpty()) {
                return response()->json(['success' => false, 'message' => 'Records not found']);
            }

            foreach ($out16Items as $out16) {
                $item_id = $out16->item_id;
                $qty_request = $out16->qty_request;
            ...
            $firstOut16 = StrOut16::where('doc_no', $doc_no)->first();
            if ($firstOut16) {
                $firstOut16->status = 1; // Example status update
                $firstOut16->save();
            }
    ...
    {
        $doc_no = $request->input('doc_no');
        $strOut16 = StrOut16::where('doc_no', $doc_no)->first();
        if ($strOut16) {
            $strOut16->status = 2;
            $strOut16->save();
    ...
    public function saveChecked(Request $request)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $itemId) {
                ...
                $item = StrOut16::find($itemId); // Ganti dengan model yang sesuai
                if ($item) {
                    // Contoh update field
            ...
            if ($updatedDocNo) {
                $itemToUpdate = StrOut16::where('doc_no', $updatedDocNo)->first();
                if ($itemToUpdate) {
