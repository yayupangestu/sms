...
use App\Models\StrOut17;
use App\Models\StrCategory;
use App\Models\StrUom;
use App\Models\User;
use Illuminate\Http\Request;
use App\Exports\ItemsOut17Export;
...
class StrOut17Controller extends Controller
{
    public function index()
    {
        ...
        return view('store.out17', compact('title', 'departements', 'str_uoms', 'master_list_strs', 'users'));
    }

    public function list()
    {
        $query = DB::table('str_out17s as a')
    ...
    public function update(Request $request)
    {
        if ($request->button == "update") {
            ...
            for ($i = 1; $i < $jml + 1; $i++) {
                ...
                StrOut17::where('id', $request->idline[$i])->where('id', $request->idket[$i])->update($data);
            }
    ...
    {
        $cek = StrOut17::select(DB::raw('COUNT(doc_no) as jml'))->whereMonth('created_at', date('m'))->groupBy('doc_no')->count();
        if ($cek > 0) {
            $array = array();
            $rows = StrOut17::select('doc_no')->whereMonth('created_at', date('m'))->groupBy('doc_no')->get();
    ...
    public function update2(Request $request)
    {
        ...
        $query = StrOut17::where('id', $request->id)->update($data);
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsOut17Export($startDate, $endDate), 'Penggunaan Barang Painting.xlsx');
    }

    public function destroyline(Request $request)
    {
        $query = StrOut17::where('id', $request->id)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    {
        $cek = StrOut17::where('id', $request->id)->count();
        if ($cek > 0) {
            $row = StrOut17::where('id', $request->id)->first();
            return response()->json([
                'success' => true,
                'id' => $row->id,
                'item_id' => $row->item_id,
    ...
    {
        $query = StrOut17::where('doc_no', $request->doc_no)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    {
        $cek = StrOut17::where('doc_no', $request->doc_no)->count();
        if ($cek > 0) {
            $data['sts'] = 1;
            $query = StrOut17::where('doc_no', $request->doc_no)->update($data);
            if ($query) {
                return response()->json([
                    'success' => true,
                    'msg' => 'Submit success.'
    ...
    {
        $row = StrOut17::select('date_plan', 'line_id')
    ...
    public function approve(Request $request)
    {
        ...
        try {
            ...
            $out17Items = StrOut17::where('doc_no', $doc_no)->get();
            ...
            $firstOut17 = StrOut17::where('doc_no', $doc_no)->first();
            if ($firstOut17) {
    ...
    {
        $doc_no = $request->input('doc_no');
        $strOut17 = StrOut17::where('doc_no', $doc_no)->first();
        if ($strOut17) {
            $strOut17->status = 2;
            $strOut17->save();
    ...
    public function saveChecked(Request $request)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $itemId) {
                ...
                $item = StrOut17::find($itemId); // Ganti dengan model yang sesuai
                if ($item) {
                    // Contoh update field
            ...
            if ($updatedDocNo) {
                $itemToUpdate = StrOut17::where('doc_no', $updatedDocNo)->first();
                if ($itemToUpdate) {
