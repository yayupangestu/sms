...
class StrStokRtkController extends Controller
{
    public function index()
    {
        ...
        $master_list_strs = DB::table('master_list_strs as a')
            ...
            ->get();
        return view('store.stokrtk', compact('title', 'str_uoms', 'master_list_strs', 'str_uoms'));
    }
    ...
    {
        $query = DB::table('str_stok_rtks as a')
    ...
    {
        $cek = StrStokRtk::where('id', $request->id)->count();
        if ($cek > 0) {
            $row = StrStokRtk::where('id', $request->id)->first();
            return response()->json([
                'success' => true,
                'id' => $row->id,
                'item_id' => $row->item_id,
    ...
    public function update(Request $request)
    {
        ...
        $query = StrStokRtk::where('id', $request->id)->update($data);
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Update success.'
    ...
    {
        $query = StrStokRtk::where('id', $request->id)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    public function export()
    {
        ...
        return Excel::download(new RtkExport, 'RtkExport.xlsx');
    }
}
