...
class Lkhb3Controller extends Controller
{
    public function index()
    {
        ...
        $tabel_b3_s = DB::table('tabel_b3_s as a')
                ...
                ->get();
        return view('planningline.lkhb3', compact('title','line_stmps','data_part_names','rm_materials','tabel_b3_s'));
    }

    public function list(Request $request)
    {
        $query = DB::table('planning_line_b3_s as a')
                ->select('a.id','e.part_name','e.job_no','d.name_material as spek', 'a.qty_plan','a.qty_act','a.qty_ng','a.qty_gsph','a.qty_mesin','a.qty_dandori','a.ket_remark','a.qty_dies','f.name')
                ->join('line_stmps as b', 'b.id', '=', 'a.line_id', 'left')
                ->join('tabel_b3_s as c', 'c.id', '=', 'a.product_id', 'left')
    ...
    public function update(Request $request)
    {
        if($request->button == "update"){
            ...
            for ($i=1; $i < $jml+1; $i++) { 
                ...
                PlanningLineB3::where('id', $request->idline[$i])->where('id', $request->idng[$i])->where('id', $request->idgsph[$i])->where('id', $request->idmesin[$i])->where('id', $request->iddies[$i])->where('id', $request->iddandori[$i])->where('id', $request->idremark[$i])->update($data);
            }
