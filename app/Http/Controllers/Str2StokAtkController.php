...
use App\Models\User;
use App\Exports\Atk2Export;
use Maatwebsite\Excel\Facades\Excel;
use Illuminate\Http\Request;
use DataTables;
use DB;

class Str2StokAtkController extends Controller
{
    public function index()
    {
        ...
        $master_list_strs = DB::table('master_list_strs as a')
            ...
            ->get();
        return view('store2.stokatk', compact('title', 'str_uoms', 'master_list_strs'));
    }
    ...
    {
        $query = DB::table('str2_stok_atks as a')
    ...
    {
        $cek = Str2StokAtk::where('id', $request->id)->count();
        if ($cek > 0) {
            $row = Str2StokAtk::where('id', $request->id)->first();
            return response()->json([
                'success' => true,
                'id' => $row->id,
                'item_id' => $row->item_id,
    ...
    public function update(Request $request)
    {
        ...
        $query = Str2StokAtk::where('id', $request->id)->update($data);
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Update success.'
    ...
    {
        $query = Str2StokAtk::where('id', $request->id)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.'
    ...
    public function export()
    {
        ...
        return Excel::download(new Atk2Export, 'AtkExport.xlsx');
    }
}
