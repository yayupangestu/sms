...
class StmpKanbanB2Controller extends Controller
{
    public function index()
    {
       ...
       $data_fg_stampings = PlanningLineB3::whereBetween('created_at', [$startTime, $endTime])
        ...
        return view('stamping.tagkanbanb2', compact('title','line_stmps','data_part_names','rm_materials','master','data_fg_stampings'));
    }

    public function list()
    {
        $query = DB::table('stmp_tag_kanban_b2_s as a')
    ...
    public function getDropdownData()
    {
        ...
        $data = DB::table('planning_line_b3_s')
            ...
            ->where('mesin', 'LINE B2') // Filter mesin
...
public function store(Request $request)
{
    ...
    {
        $cek = StmpTagKanbanB2::where('id', $request->id)->count();
        if($cek > 0){
            $row = StmpTagKanbanB2::where('id', $request->id)->first();
            return response()->json([
                'success'        => true,
                'id'             => $row->id,
    ...
    public function update(Request $request)
    {
        ...
        $query = StmpTagKanbanB2::where('id', $request->id)->update($data);
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Edit success.'
    ...
    {
        $query = StmpTagKanbanB2::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $query = StmpTagKanbanB2::where('date_plan', $request->date_plan)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    // {
    //     $cek = StmpTagKanbanB2::select(DB::raw('COUNT(doc_no) as jml'))->whereMonth('created_at', date('m'))->groupBy('doc_no')->count();
    //     if($cek > 0){
    //         $array = array();
    //         $rows = StmpTagKanbanB2::select('doc_no')->whereMonth('created_at', date('m'))->groupBy('doc_no')->get();
    ...
    {
        $cek = StmpTagKanbanB2::where('date_plan', $request->date_plan)->count();
        if($cek > 0){
            $data['sts'] = 1;
            $query = StmpTagKanbanB2::where('date_plan', $request->date_plan)->update($data);
            if($query){
                return response()->json([
                    'success'   => true,
                    'msg'       => 'Submit success.'
    ...
    public function cetak($id)
    {
        ...
        $master = StmpTagKanbanB2::findOrFail($id);
      ...
      $data_to_encode = "{$detail->part_no2}.{$detail->job_no}.{$detail->qty_ok}.{$detail->id}.{$detail->uniqNo}.{$detail->model}.{$detail->part_no}.{$detail->date_plan}.{$detail->kode_material}.{$detail->qty_ng}";
        ...
        $pdf = PDF::loadView('formatkanban.kanbanstmpb2', $data)->setPaper($customPaper, 'potrait');
