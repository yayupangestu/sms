...
use App\Exports\PlanningC1Export;

class DashboardPlanningC1 extends Controller
{
    public function index()
{
    $title = 'Dashboard C1 C2 3';
    ...
    return view('dashboard2.dashboardc1', compact(
        'title', 'planningData', 'scan_in_labels', 'trace_abilities',
        'rm_stoks', 'rm_return_materials', 'scan_out_rms'
    ));
}
    ...
    public function getLatestPlanningData(Request $request)
{
    ...
    $query = DB::table('planning_line_b3_s')
        ...
        ->whereIn('mesin', ['LINE B3', 'LINE C1', 'LINE C2', 'KOMATSU', 'FUKUI', 'AMINO'])
...
//         ->where('line_id', 1); // ✅ hanya update line_id = 1
...
public function cancelProduction(Request $request)
{
    ...
    if ((int)$request->status_proses === 2) {
        // Utamakan mesin LINE C1
        $queryC1 = PlanningLineB3::where('part_no', $request->part_no)
            ->where('date_plan', $request->date_plan)
            ->where('shift', $request->shift)
            ->where('mesin', 'LINE C1')
            ->where('status_proses', '!=', 3);

        $updatedC1 = $queryC1->update($dataToUpdate);
        $updated += $updatedC1;

        // Kalau tidak ada di LINE C1 → coba LINE C2
        if ($updatedC1 === 0) {
            $queryC2 = PlanningLineB3::where('part_no', $request->part_no)
                ->where('date_plan', $request->date_plan)
                ->where('shift', $request->shift)
                ->where('mesin', 'LINE C2')
                ->where('status_proses', '!=', 3);

            $updatedC2 = $queryC2->update($dataToUpdate);
            $updated += $updatedC2;
        }

        // Kalau bukan di C1 atau C2 → cek LINE B3 juga
        if ($updated === 0) {
            $queryB3 = PlanningLineB3::where('part_no', $request->part_no)
                ->where('date_plan', $request->date_plan)
                ->where('shift', $request->shift)
                ->where('mesin', 'LINE B3')
                ->where('status_proses', '!=', 3);

            $updatedB3 = $queryB3->update($dataToUpdate);
            $updated += $updatedB3;
        }
    } else {
        ...
        $query = PlanningLineB3::where('part_no', $request->part_no)
            ...
            ->where(function ($q) {
                $q->where('line_id', 1)      // default
                  ->orWhereIn('mesin', ['LINE B3', 'LINE C1', 'LINE C2']); // tambahan fleksibel
            });
...
public function cekProsesMesin(Request $request)
{
    ...
    //         ->whereIn('mesin', ['LINE B3', 'LINE C1', 'LINE C2'])
...
public function accepted2(Request $request)
{
    ...
    public function lineB3Data()
    {
        try {
            $start = Carbon::today()->addHours(7);
            $end = Carbon::tomorrow()->addHours(7)->addMinutes(30);

            $data_transit = TabelTransitMaterial::where('mesin', 'LINE B3')
    ...
    ->map(function ($item) use ($start, $end) {
        ...
        return [
            ...
            \Log::error('Gagal ambil data LINE B3: ' . $e->getMessage());
            return response()->json(['error' => 'Terjadi kesalahan server.'], 500);
        }
    }

    public function lineC1Data()
    {
        try {
            $start = Carbon::today()->addHours(7);
            $end = Carbon::tomorrow()->addHours(7)->addMinutes(30);

            $data_transit = TabelTransitMaterial::where('mesin', 'LINE C1')
    ->where(function ($query) {
        $query->where(function ($q) {
            $q->where('sts', 1)
              ->where('sts_proses', 2);
    ...
    ->map(function ($item) use ($start, $end) {
        ...
        return [
            ...
            \Log::error('Gagal ambil data LINE C1: ' . $e->getMessage());
            return response()->json(['error' => 'Terjadi kesalahan server.'], 500);
        }
    }

    public function lineC2Data()
    {
        try {
            $start = Carbon::today()->addHours(7);
            $end = Carbon::tomorrow()->addHours(7)->addMinutes(30);

            $data_transit = TabelTransitMaterial::where('mesin', 'LINE C2')
    ->where(function ($query) {
        $query->where(function ($q) {
            $q->where('sts', 1)
              ->where('sts_proses', 2);
    ...
    public function getDiesHistory(Request $request)
    {
        ...
        $runningData = DB::table('planning_line_b3_s')
            ...
            ->where('status_proses', 2)
            ->whereIn('mesin', ['LINE C1', 'LINE C2', 'LINE B3'])
    ...
    public function approveProduction(Request $request)
    {
        try {
            ...
            // $planningItem->user_stamping_done = auth()->user()->username; // Use this if auth is available
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new PlanningC1Export($startDate, $endDate, $mesin), 'Planning_C1_Export_' . $startDate . '_to_' . $endDate . '.xlsx');
    }
}
