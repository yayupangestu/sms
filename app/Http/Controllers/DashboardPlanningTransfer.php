...
class DashboardPlanningTransfer extends Controller
{
    public function index()
    {
        ...
        $planningData = DB::table('planning_line_b3_s as a')->select('a.id', 'a.product_id', 'a.job_no', 'a.part_no', 'a.qty_plan', 'a.name_material')->get();
        ...
        return view('dashboard2.dashboardtransfer', compact('title', 'planningData', 'scan_in_labels', 'trace_abilities', 'rm_stoks', 'rm_return_materials', 'scan_out_rms'));
    }
    ...
    public function getIncomingMaterials(Request $request)
{
    ...
    public function getLatestRmReturnMaterials(Request $request)
    {
        ...
        $materials = ScanOutRm::whereDate('created_at', $today)->orderBy('created_at', 'desc')->get();
    ...
    public function cancelProduction(Request $request)
    {
        ...
        if ((int) $request->status_proses === 2) {
            ...
            $queryA1 = PlanningLineB3::where('part_no', $request->part_no)->where('date_plan', $request->date_plan)->where('shift', $request->shift)->where('mesin', 'TRANSFERS')->where('status_proses', '!=', 3);
            ...
            if ($updatedA1 === 0) {
                $queryA2 = PlanningLineB3::where('part_no', $request->part_no)->where('date_plan', $request->date_plan)->where('shift', $request->shift)->where('mesin', 'TRANSFERS')->where('status_proses', '!=', 3);
            ...
            $query = PlanningLineB3::where('part_no', $request->part_no)
                ...
                ->where(function ($q) {
                    $q->where('line_id', 1) // default
                        ->orWhereIn('mesin', ['TRANSFERS']); // tambahan fleksibel
                });
    ...
    public function cekProsesMesin(Request $request)
    {
        ...
        $ada = PlanningLineB3::where('mesin', $mesin)->where('status', 2)->where('status_proses', 2)->exists();
    ...
    public function lineTransfersData()
    {
        try {
            $start = Carbon::today()->addHours(7);
            $end = Carbon::tomorrow()->addHours(7)->addMinutes(30);

            $data_transit = TabelTransitMaterial::where('mesin', 'TRANSFERS')
                ->where(function ($query) {
                    $query
                        ->where(function ($q) {
                            $q->where('sts', 1)->where('sts_proses', 2);
                        })
                        ->orWhere(function ($q) {
                            $q->where('sts', 1)->where('sts_proses', 1);
                        });
                })
                ->where('qty_stamping', '>', 0) // 👈 PENTING: filter pasti

                ->where(function ($query) {
                    $query->where('sts_proses', '!=', 2)->orWhere(function ($sub) {
                        $sub->where('sts_proses2', '!=', 3)->orWhere('qty_stamping', '>', 0);
                    });
                })
                ->where(function ($q) {
                    $q->whereNull('sts_proses2')
                        ...
                        ->orWhere(function ($sub) {
                            $sub->where('sts_proses2', 3)->where('qty_stamping', '>', 0);
                        });
                })
            ...
            \Log::error('Gagal ambil data TRANSFERS: ' . $e->getMessage());
