...
class LkhC1Controller extends Controller
{
    public function index()
    {
        ...
        $tabel_c1_s = DB::table('tabel_c1_s as a')
                ...
                ->get();
        return view('planningline.lkhc1', compact('title','line_stmps','data_part_names','rm_materials','tabel_c1_s'));
    }

    public function list(Request $request)
    {
        $query = DB::table('planning_line_c1_s as a')
                ...
                ->join('tabel_c1_s as c', 'c.id', '=', 'a.product_id', 'left')
    ...
    public function update(Request $request)
    {
        if($request->button == "update"){
            ...
            for ($i=1; $i < $jml+1; $i++) { 
                ...
                PlanningLineC1::where('id', $request->idline[$i])->where('id', $request->idng[$i])->where('id', $request->idgsph[$i])->where('id', $request->idmesin[$i])->where('id', $request->iddies[$i])->where('id', $request->iddandori[$i])->where('id', $request->idremark[$i])->update($data);
            }
    	    ...
    	    return $pdf->download('lkhc1.pdf');
        }
    }

    public function destroy(Request $request)
    {
        $query = Lkhc1::where('date_plan', $request->date_plan)->where('line_id', $request->idline)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
