...
class StmpTagKanbanController extends Controller
{
    public function index()
    {
        ...
        return view('stamping.tagkanban', compact('title','line_stmps','data_part_names','rm_materials','master','data_fg_stampings'));
    }

    public function list()
    {
        $query = DB::table('stmp_tag_kanbans as a')
                ->select('a.date_plan','b.name as line',DB::raw('CONCAT(a.date_plan, b.id)as mix_id'))
                ->join('line_stmps as b', 'b.id', '=', 'a.line_id', 'left')
                ->groupBy('a.doc_no', 'a.date_plan', 'a.line_id', 'b.id')
                ->get();
    ...
    public function getUniqNosByMachine(Request $request)
    {
        ...
        // Ambil data rm_uniqNo dan rm_uniqNo2 berdasarkan mesin (LINE B3) dan urutkan berdasarkan created_at
        $uniqNos = DB::table('planning_line_b3_s')
        ->where('mesin', $request->input('mesin'))  // Filter berdasarkan mesin (LINE B3)
    ...
    public function getDropdownData()
    {
        ...
        $data = DB::table('planning_line_b3_s')
            ...
            ->where('mesin', 'LINE B3') // Filter mesin
...
public function store(Request $request)
{
    ...
    {
        $cek = StmpTagKanban::where('id', $request->id)->count();
        if($cek > 0){
            $row = StmpTagKanban::where('id', $request->id)->first();
            return response()->json([
                'success'        => true,
                'id'             => $row->id,
    ...
    public function update(Request $request)
    {
        ...
        $data['part_no_rm']         = $request->part_no_rm;
        $data['tujuan']         = $request->tujuan;
        $data['kode_material']  = $request->kode_material;
        $data['keterangan']     = $request->keterangan;
        $data['updateby']       = auth()->user()->id;
        $query = StmpTagKanban::where('id', $request->id)->update($data);
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Edit success.'
    ...
    {
        $query = StmpTagKanban::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $query = StmpTagKanban::where('date_plan', $request->date_plan)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    // {
    //     $cek = StmpTagKanban::select(DB::raw('COUNT(doc_no) as jml'))->whereMonth('created_at', date('m'))->groupBy('doc_no')->count();
    //     if($cek > 0){
    //         $array = array();
    //         $rows = StmpTagKanban::select('doc_no')->whereMonth('created_at', date('m'))->groupBy('doc_no')->get();
    ...
    {
        $cek = StmpTagKanban::where('date_plan', $request->date_plan)->count();
        if($cek > 0){
            $data['sts'] = 1;
            $query = StmpTagKanban::where('date_plan', $request->date_plan)->update($data);
            if($query){
                return response()->json([
                    'success'   => true,
                    'msg'       => 'Submit success.'
    ...
    //     $master = StmpTagKanban::findOrFail($id);
    ...
    //     $query = DB::table('stmp_tag_kanbans as a')
    //             ->select('a.id', 'a.part_name','a.part_no','a.part_no2','a.job_no','a.model','a.name_material', 'a.qty_ok', 'a.qty_ng','a.model', 'a.time_start', 'a.time_end', 'a.shift', 'a.uniqNo','g.username as createdby','a.shift','a.date_plan','a.tujuan','b.name as line_id','a.line','a.kode_material','a.part_no_rm')
    //             ->join('line_stmps as b', 'b.id', '=', 'a.line_id', 'left')
    ...
    //     $data_to_encode = "{$detail->part_no2}.{$detail->part_no}.{$detail->job_no}.{$detail->model}.{$detail->qty_ok}.{$detail->date_plan}.{$detail->uniqNo}.{$detail->kode_material}.{$detail->qty_ng}.{$detail->id}";
    ...
    //         'listdetail'        => $query, // Kirimkan hasil query ke view

    //     ];
    //     $customPaper = [0, 0, 446.93, 302.14]; // 200mm x 130mm in points


    //     // Load view and generate PDF in landscape
    //     $pdf = PDF::loadView('formatkanban.kanbanstmp', $data)->setPaper($customPaper, 'potrait');
    ...
    public function generateMultipleQrCodes(Request $request)
    {
        ...
        if (!$ids || !is_array($ids) || count($ids) === 0) {
            return redirect()->back()->with('error', 'ID tidak ditemukan dalam request.');
        }

        // Ambil semua data master
        $masters = StmpTagKanban::whereIn('id', $ids)->get();

        // Ambil semua data detail
        $query = DB::table('stmp_tag_kanbans as a')
            ->select(
                'a.id', 'a.part_name','a.part_no','a.part_no2','a.job_no','a.model',
                'a.name_material', 'a.qty_ok', 'a.qty_ng', 'a.time_start',
                'a.time_end', 'a.shift', 'a.uniqNo', 'g.username as createdby',
                'a.date_plan','a.tujuan','b.name as line_id','a.line',
                'a.kode_material','a.part_no_rm'
            )
            ->leftJoin('line_stmps as b', 'b.id', '=', 'a.line_id')
            ->leftJoin('users as g', 'g.id', '=', 'a.createdby')
            ->whereIn('a.id', $ids) // ✅ Perbaikan di sini
            ->get();
        ...
        // Generate QR code per detail
        $qrcodes = [];

        foreach ($query as $detail) {
            $data_to_encode = "{$detail->part_no2}.{$detail->part_no}.{$detail->job_no}.{$detail->model}.{$detail->qty_ok}.{$detail->date_plan}.{$detail->uniqNo}.{$detail->kode_material}.{$detail->qty_ng}.{$detail->id}";
            $qrcodes[] = base64_encode(
                QrCode::format('svg')->size(100)->errorCorrection('H')->generate($data_to_encode)
            );
        }

        // Data yang dikirim ke view
        $data = [
            'UniqNo'     => optional($masters->first())->uniqNo ?? '',
            'line_stmps' => $masters->first(),
            'qrcode'     => optional($masters->first())->qrcode ?? '',
            'no'         => optional($masters->first())->no ?? '',
            'part_no_rm' => optional($masters->first())->part_no_rm ?? '',
            'line'       => optional($masters->first())->line ?? '',
            'line_id'    => optional($masters->first())->line_id ?? '',
            'qty_ok'     => optional($masters->first())->qty_ok ?? '',
            'listdetail' => $query,
            'qrcodes'    => $qrcodes, // ✅ TAMBAHKAN INI
        ];


        $customPaper = [0, 0, 446.93, 302.14];

        $pdf = PDF::loadView('formatkanban.kanbanstmp2', $data)->setPaper($customPaper, 'portrait');

        return $pdf->stream(date('d_M_Y') . '_qrcode.pdf');
    }
