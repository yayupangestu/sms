...
class StrKanbanController extends Controller
{
    public function index()
    {
        $title = 'Kanban 1';
        $users = User::all();
        $master_list_strs = MasterListStr::all();
        $str_uoms = StrUom::all();
        ...
        $str_out18s = DB::table('str_out18s as a')
            ->select('a.doc_no', 'a.status', 'a.w_diberi', 'b.name as createdby')
            ->join('users as b', 'b.id', '=', 'a.createdby', 'left')
            ->whereIn('status', [1, 2]) // Filter data dengan status 1 dan 2 saja
            ->get();
        $str_out18s = [
            'status33' => $str_out18s->where('status', 1),
            'status34' => $str_out18s->where('status', 2)
        ];

        $str_out19s = DB::table('str_out19s as a')
            ->select('a.doc_no', 'a.status', 'a.w_diberi', 'b.name as createdby')
            ->join('users as b', 'b.id', '=', 'a.createdby', 'left')
            ->whereIn('status', [1, 2]) // Filter data dengan status 1 dan 2 saja
            ->get();
        $str_out19s = [
            'status35' => $str_out19s->where('status', 1),
            'status36' => $str_out19s->where('status', 2)
        ];

        $str_out20s = DB::table('str_out20s as a')
            ->select('a.doc_no', 'a.status', 'a.w_diberi', 'b.name as createdby')
            ->join('users as b', 'b.id', '=', 'a.createdby', 'left')
            ->whereIn('status', [1, 2]) // Filter data dengan status 1 dan 2 saja
            ->get();
        $str_out20s = [
            'status37' => $str_out20s->where('status', 1),
            'status38' => $str_out20s->where('status', 2)
        ];


        return view('storedashboard.kanban', compact('title', 'str_out2s', 'users', 'str_uoms', 'str_out3s', 'str_out4s', 'str_out5s', 'str_out6s', 'str_out7s', 'str_out8s', 'str_out9s', 'str_out11s', 'str_out10s', 'str_out12s', 'str_out13s', 'str_out14s', 'str_out15s', 'str_out16s', 'str_out17s', 'str_out18s', 'str_out19s', 'str_out20s'));
    }
    ...
    public function getDetails(Request $request)
    {
        ...
        $query1 = DB::table('str_out2s as a')
            ...
            ->join('master_list_strs as b', 'b.id', '=', 'a.item_id')
        ...
        $query17 = DB::table('str_out18s as a')
            ->select('b.name as item_id', 'a.w_diberi', 'a.qty_request', 'c.name as satuan')
            ->join('master_list_strs as b', 'b.id', '=', 'a.item_id')
            ->join('str_uoms as c', 'c.id', '=', 'a.satuan')
            ->where('doc_no', $docNo);
        $query18 = DB::table('str_out19s as a')
            ->select('b.name as item_id', 'a.w_diberi', 'a.qty_request', 'c.name as satuan')
            ->join('master_list_strs as b', 'b.id', '=', 'a.item_id')
            ->join('str_uoms as c', 'c.id', '=', 'a.satuan')
            ->where('doc_no', $docNo);

        $query19 = DB::table('str_out20s as a')
            ->select('b.name as item_id', 'a.w_diberi', 'a.qty_request', 'c.name as satuan')
            ->join('master_list_strs as b', 'b.id', '=', 'a.item_id')
            ->join('str_uoms as c', 'c.id', '=', 'a.satuan')
            ->where('doc_no', $docNo);
        ...
        $docDetails = $query1->union($query2)->union($query3)->union($query4)->union($query5)->union($query6)->union($query7)->union($query8)->union($query9)->union($query10)->union($query11)->union($query12)->union($query13)->union($query14)->union($query15)->union($query16)->union($query17)->union($query18)->union($query19)->get();
    ...
    public function getNewData()
    {
        ...
        // Organize str_out3s data into status groups
        $str_out18s = [
            'status33' => $str_out18s->where('status', 1)->values(),
            'status34' => $str_out18s->where('status', 2)->values(),
        ];
        ...
        // Organize str_out3s data into status groups
        $str_out19s = [
            'status35' => $str_out19s->where('status', 1)->values(),
            'status36' => $str_out19s->where('status', 2)->values(),
        ];
        ...
        // Organize str_out3s data into status groups
        $str_out20s = [
            'status37' => $str_out20s->where('status', 1)->values(),
            'status38' => $str_out20s->where('status', 2)->values(),
        ];
        ...
        return response()->json([
            ...
            'str_out18s' => $str_out18s,
            'str_out19s' => $str_out19s,
            'str_out20s' => $str_out20s,
        ]);
    }
