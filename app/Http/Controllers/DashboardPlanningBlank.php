...
class DashboardPlanningBlank extends Controller
{
    public function index()
{
    $title = 'Dashboard Blanl';
   ...
   $data_fg_stampings = PlanningLineB3::whereBetween('created_at', [$startTime, $endTime])
    ...
    return view('blank.dashboardproses', compact('line_stmps','data_part_names','data_fg_stampings',
        'title', 'scan_in_labels'
    ));
}
        ...
        $query = PlanningLineB3::where('part_no', $request->part_no)
            ...
            ->where(function ($q) {
                $q->where('line_id', 1)      // default
                  ->orWhereIn('mesin', ['KOMATSU','FUKUI']); // tambahan fleksibel
            });
...
public function accepted2(Request $request)
{
    ...
    public function lineKomatsuData()
    {
        try {
            $start = Carbon::today()->addHours(7);
            $end = Carbon::tomorrow()->addHours(7)->addMinutes(30);

            $data_transit = TabelTransitMaterial::where('mesin', 'KOMATSU')
    ...
    ->map(function ($item) use ($start, $end) {
        ...
        return [
            ...
            \Log::error('Gagal ambil data KOMATSU: ' . $e->getMessage());
            return response()->json(['error' => 'Terjadi kesalahan server.'], 500);
        }
    }

    public function lineFukuiData()
    {
        try {
            $start = Carbon::today()->addHours(7);
            $end = Carbon::tomorrow()->addHours(7)->addMinutes(30);

            $data_transit = TabelTransitMaterial::where('mesin', 'FUKUI')
    ->where(function ($query) {
        $query->where(function ($q) {
            $q->where('sts', 1)
              ->where('sts_proses', 2);
    ...
    ->map(function ($item) use ($start, $end) {
        ...
        return [
            ...
            \Log::error('Gagal ambil data FUKUI: ' . $e->getMessage());
            return response()->json(['error' => 'Terjadi kesalahan server.'], 500);
        }
    }
