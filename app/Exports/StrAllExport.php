...
class StrAllExport implements FromCollection, WithTitle, ShouldAutoSize, WithHeadings, WithEvents, WithMapping
{
    protected $startDate;
    protected $endDate;
    ...
    public function collection()
    {
        ...
        $results = $query1->union($query2)->union($query3)->union($query4)->union($query5)->union($query6)->get();
    ...
    {
        return [
            ["Data Total Item-Out A1, A2, B1, B2, B3, C1, C2, TRANSFERS, AMINO, FUKUI, KOMATSU  "],
            [""],
            ["No", "Doc No", "Line", "Item Name", "Qty Return", "Qty Request", "Qty Out", "Satuan", "Date", "Harga"],
        ];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->getDelegate()->getStyle('A3:J3')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
                $event->sheet->getDelegate()->getStyle('A3:J3')->getFont()->setBold(true);
                $event->sheet->getDelegate()->getStyle('A3:J3')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setRGB('D3D3D3'); // Light Grey background for header
            },
        ];
    }
}
