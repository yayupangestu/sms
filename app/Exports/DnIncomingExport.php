...
class DnIncomingExport implements FromCollection, WithTitle, ShouldAutoSize, WithHeadings, WithEvents, WithMapping
{
    private $rowNumber = 1;
    private $supplierFilter;
    private $docPoFilter;
    private $periodeFilter;

    // Constructor to initialize filters
    public function __construct($supplierFilter, $docPoFilter, $periodeFilter)
    {
        $this->supplierFilter = $supplierFilter;
        $this->docPoFilter = $docPoFilter;
        $this->periodeFilter = $periodeFilter;
    }

    public function collection()
    {
        // Split filters by comma, only if they are not empty
        $suppliers = $this->supplierFilter ? explode(',', $this->supplierFilter) : [];
        $docPos = $this->docPoFilter ? explode(',', $this->docPoFilter) : [];
        $periodes = $this->periodeFilter ? explode(',', $this->periodeFilter) : [];
    
        // Build the query with optional filters
        $query = RmDnIncoming::query();
        ...
        if (!empty($docPos)) {
            $query->whereIn('doc_po', $docPos);
        }
    
        if (!empty($periodes)) {
            $query->whereIn('created_at', $periodes);
        }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $event->sheet->getDelegate()->getStyle('P4:P' . ($event->sheet->getHighestRow()))
                    ->getFill()
                    ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
                    ->getStartColor()->setRGB('90EE90'); // Light Green
    
                $event->sheet->getDelegate()->getStyle('R4:R' . ($event->sheet->getHighestRow()))
                    ->getFill()
                    ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
                ...
                // Add borders around data (A3 to last row in column W)
                ...
                // Check if status in column V is "Close" and color it green
                $highestRow = $event->sheet->getHighestRow();
                for ($row = 4; $row <= $highestRow; $row++) {
                    $status = $event->sheet->getCell('V' . $row)->getValue();
                    if ($status === 'Close') {
                        $event->sheet->getDelegate()->getStyle('V' . $row)
                            ->getFill()
                            ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
                            ->getStartColor()->setRGB('008000'); // Green color
                    }
                }
            },
        ];
