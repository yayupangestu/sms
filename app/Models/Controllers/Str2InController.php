...
use App\Models\Str2In;
use App\Models\StrCategory;
use App\Models\StrUom;
use App\Models\StrSuplaier;
use Illuminate\Http\Request;
use App\Exports\ItemsInExportStr2;
use Maatwebsite\Excel\Facades\Excel;
use DataTables;
use DB;

class Str2InController extends Controller
{
    public function index()
    {
        ...
        $str_barangs = DB::table('str_barangs as a')
                    ...
                    ->get();
        return view('storeroom2.strin', compact('title','str_uoms','str_barangs','str_categories','str_suplaiers'));
    }

    public function list()
    {
        $query = DB::table('str2_ins as a')
    ...
    {
        $query = Str2In::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $query = Str2In::where('date_plan', $request->date_plan)->where('category_id', $request->idcategory)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsInExportStr2($startDate, $endDate), 'items_in.xlsx');
    }
}
