...
use App\Models\StrInBecak;
use App\Models\StrCategory;
use App\Models\StrUom;
use App\Models\StrSuplaier;
use Illuminate\Http\Request;
use App\Exports\ItemsInBecakExport;
use Maatwebsite\Excel\Facades\Excel;
use DataTables;
use DB;

class StrInBecakController extends Controller
{
    public function index()
    {
        $title = 'Item IN Becak';
        $str_categories = StrCategory::where('id', 12)->get();
       ...
       $str_barangs = DB::table('str_barangs as a')
            ...
            ->where('a.category', '=', 12)
            ->get();
        return view('storeroom.strinbecak', compact('title','str_uoms','str_barangs','str_categories','str_suplaiers'));
    }

    public function list()
    {
        $query = DB::table('str_in_becaks as a')
    ...
    // {
    //     $query = StrInBecak::where('id', $request->id)->delete();
    //     if($query){
    //         return response()->json([
    //             'success'   => true,
    //             'msg'       => 'Delete success.'
     ...
     public function destroyline(Request $request)
    {
        ...
        try {
            ...
            $strIn = StrInBecak::find($request->id);
            ...
            // Ambil stok terkait item_id dari tabel str_stok_atks
            $stok = DB::table('str_stok_becaks')
                ->where('item_id', $strIn->item_id)
                ->first();
    
            if ($stok && isset($stok->actual)) {
                // Kurangi stok berdasarkan qty_in dari data yang dihapus
                $newQty = max(0, $stok->actual - $strIn->qty_in);
    
                DB::table('str_stok_becaks')
                    ->where('item_id', $strIn->item_id)
                    ->update(['actual' => $newQty]);
            }
    ...
    {
        $query = StrInBecak::where('date_plan', $request->date_plan)->where('category_id', $request->idcategory)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsInBecakExport($startDate, $endDate), 'Item In Becak.xlsx');
    }
}
