...
use App\Exports\ItemsOut4Export;
use App\Exports\ItemsOut4SummaryExport;
...
class StrOut4Controller extends Controller
{
    public function index()
    {
        ...
        return view('storeroom.out4', compact('title', 'departements', 'str_uoms', 'str_barangs', 'users'));
    }
    ...
    //     return view('storeroom.out2', compact('title', 'departements', 'str_uoms', 'str_barangs', 'users'));
    // }

    public function list()
    {
        $query = DB::table('str_out4s as a')->select('a.date_plan', 'a.doc_no', 'a.line_id', 'c.name as createdby', DB::raw('GROUP_CONCAT(a.status) as status'), DB::raw('MAX(a.created_at) as created_at'))->join('users as c', 'c.id', '=', 'a.createdby', 'left')->where('a.sts', 1)->groupBy('a.doc_no', 'a.date_plan', 'a.line_id', 'a.createdby')->orderBy('created_at', 'desc')->get();
    ...
    public function update(Request $request)
    {
        if ($request->button == 'update') {
            ...
            for ($i = 1; $i < $jml + 1; $i++) {
                ...
                StrOut4::where('id', $request->idline[$i])->where('id', $request->idket[$i])->update($data);
            }
    ...
    {
        $query = DB::table('str_out4s as a')->select('a.id', 'a.date_plan', 'c.name', 'a.qty_return', 'a.qty_standing', 'a.qty_request', 'a.keterangan', 'd.name as satuan', 'a.line_id', 'a.qty_out', 'a.keterangan', 'a.w_dibuat', 'a.status_checklist', 'a.update_checklist', 'f.username as createdby')->join('str_barangs as c', 'c.id', '=', 'a.item_id', 'left')->join('str_uoms as d', 'd.id', '=', 'a.satuan', 'left')->join('departements as e', 'e.id', '=', 'a.line_id', 'left')->join('users as f', 'f.id', '=', 'a.createdby')->where('a.doc_no', $request->doc_no)->get();
    ...
    public function getdoc()
    {
        ...
        $lastDoc = StrOut4::where('doc_no', 'like', "%/$yearMonth/%")
    ...
    public function update2(Request $request)
    {
        ...
        $query = StrOut4::where('id', $request->id)->update($data);
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsOut4Export($startDate, $endDate), 'Penggunaan Barang B1 & B2.xlsx');
    }
    ...
    public function exportSummary(Request $request)
    {
        ...
        return Excel::download(new ItemsOut4SummaryExport($start, $end), 'Summary Item Line B1 B2.xlsx');
    }

    public function destroyline(Request $request)
    {
        $query = StrOut4::where('id', $request->id)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.',
    ...
    {
        $cek = StrOut4::where('id', $request->id)->count();
        if ($cek > 0) {
            $row = StrOut4::where('id', $request->id)->first();
            return response()->json([
                'success' => true,
                'id' => $row->id,
                'item_id' => $row->item_id,
    ...
    {
        $query = StrOut4::where('doc_no', $request->doc_no)->delete();
        if ($query) {
            return response()->json([
                'success' => true,
                'msg' => 'Delete success.',
    ...
    {
        $cek = StrOut4::where('doc_no', $request->doc_no)->count();
        if ($cek > 0) {
            $data['sts'] = 1;
            $query = StrOut4::where('doc_no', $request->doc_no)->update($data);
            if ($query) {
                return response()->json([
                    'success' => true,
                    'msg' => 'Submit success.',
    ...
    {
        $row = StrOut4::select('date_plan', 'line_id')->where('doc_no', $request->doc_no)->first();
        return response()->json([
    ...
    public function approve(Request $request)
    {
        ...
        try {
            ...
            $firstOut4 = StrOut4::where('doc_no', $doc_no)->first();
            if ($firstOut4) {
                $firstOut4->status = 1; // Example status update
                $firstOut4->save();
            }
...
//             $out3Items = StrOut3::where('doc_no', $doc_no)->get();
    
//             if ($out3Items->isEmpty()) {
//                 return response()->json([
//                     'success' => false,
...
//             StrOut3::where('doc_no', $doc_no)->update([
//                 'status' => 1
//             ]);
...
//     }
    ...
    {
        $doc_no = $request->input('doc_no');
        $strOut4 = StrOut4::where('doc_no', $doc_no)->first();
        if ($strOut4) {
            $strOut4->status = 2;
            $strOut4->save();
    ...
    public function saveChecked(Request $request)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $itemId) {
                ...
                $item = StrOut4::find($itemId); // Ganti dengan model yang sesuai
                if ($item) {
                    // Contoh update field
            ...
            if ($updatedDocNo) {
                $itemToUpdate = StrOut4::where('doc_no', $updatedDocNo)->first();
                if ($itemToUpdate) {
