...
use App\Models\StrOut8;
use App\Models\StrCategory;
use App\Models\StrUom;
use App\Models\User;
use Illuminate\Http\Request;
use App\Exports\ItemsOut8Export;
...
class StrOut8Controller extends Controller
{
    public function index()
    {
        ...
        return view('storeroom.out8', compact('title', 'departements', 'str_uoms', 'str_barangs', 'users'));
    }

  public function list()
    {
        $query = DB::table('str_out8s as a')
            ->select(
                'a.date_plan',
                'a.doc_no',
                'a.line_id',
            ...
            ->leftJoin('users as c', 'c.id', '=', 'a.createdby')
            ->where('a.sts', 1)
    ...
    public function getdoc()
{
    ...
    $lastDoc = StrOut8::where('doc_no', 'like', "%/$yearMonth/%")
        ->orderBy('created_at', 'desc')
        ->value('doc_no');
    ...
    public function update2(Request $request)
    {
        ...
        $query = StrOut8::where('id', $request->id)->update($data);
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsOut8Export($startDate, $endDate), 'Penggunaan Barang PSW ROBOT.xlsx');
    }

    public function destroyline(Request $request)
    {
        $query = StrOut8::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = StrOut8::where('id', $request->id)->count();
        if($cek > 0){
            $row = StrOut8::where('id', $request->id)->first();
            return response()->json([
                'success'        => true,
                'id'             => $row->id,
                'item_id'        => $row->item_id,
    ...
    {
        $query = StrOut8::where('doc_no', $request->doc_no)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = StrOut8::where('doc_no', $request->doc_no)->count();
        if($cek > 0){
            $data['sts'] = 1;
            $query = StrOut8::where('doc_no', $request->doc_no)->update($data);
            if($query){
                return response()->json([
                    'success'   => true,
                    'msg'       => 'Submit success.'
    ...
    {
        $row = StrOut8::select('date_plan','line_id')
    ...
    public function approve(Request $request)
    {
        ...
        try {
            ...
            $out8Items = StrOut8::where('doc_no', $doc_no)->get();
            ...
            $firstOut8 = StrOut8::where('doc_no', $doc_no)->first();
            if ($firstOut8) {
    ...
    {
        $doc_no = $request->input('doc_no');
        $strOut8 = StrOut8::where('doc_no', $doc_no)->first();
        if ($strOut8) {
            $strOut8->status = 2;
            $strOut8->save();
    ...
    public function saveChecked(Request $request)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $itemId) {
                ...
                $item = StrOut8::find($itemId); // Ganti dengan model yang sesuai
                if ($item) {
                    // Contoh update field
            ...
            if ($updatedDocNo) {
                $itemToUpdate = StrOut8::where('doc_no', $updatedDocNo)->first();
                if ($itemToUpdate) {
