...
use App\Exports\ItemsOut7Export;
use App\Exports\ItemsOut7SummaryExport;
...
class StrOut7Controller extends Controller
{
    public function index()
    {
        ...
        return view('storeroom.out7', compact('title', 'departements', 'str_uoms', 'str_barangs', 'users'));
    }

    public function list()
    {
        $query = DB::table('str_out7s as a')
    ...
    public function update(Request $request)
    {
        if($request->button == "update"){
            ...
            for ($i=1; $i < $jml+1; $i++) { 
                ...
                StrOut7::where('id', $request->idline[$i])->where('id', $request->idket[$i])->update($data);
            }
    ...
    public function getdoc()
{
    ...
    $lastDoc = StrOut7::where('doc_no', 'like', "%/$yearMonth/%")
        ->orderBy('created_at', 'desc')
        ->value('doc_no');
    ...
    public function update2(Request $request)
    {
        ...
        $query = StrOut7::where('id', $request->id)->update($data);
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsOut7Export($startDate, $endDate), 'Penggunaan Barang Blank.xlsx');
    }
          ...
          public function exportSummary(Request $request)
{
    ...
    return Excel::download(new ItemsOut7SummaryExport($start, $end), 'Summary Item Line Blank.xlsx');
}

    public function destroyline(Request $request)
    {
        $query = StrOut7::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = StrOut7::where('id', $request->id)->count();
        if($cek > 0){
            $row = StrOut7::where('id', $request->id)->first();
            return response()->json([
                'success'        => true,
                'id'             => $row->id,
                'item_id'        => $row->item_id,
    ...
    {
        $query = StrOut7::where('doc_no', $request->doc_no)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = StrOut7::where('doc_no', $request->doc_no)->count();
        if($cek > 0){
            $data['sts'] = 1;
            $query = StrOut7::where('doc_no', $request->doc_no)->update($data);
            if($query){
                return response()->json([
                    'success'   => true,
                    'msg'       => 'Submit success.'
    ...
    {
        $row = StrOut7::select('date_plan','line_id')
    ...
    //         $out7Items = StrOut7::where('doc_no', $doc_no)->get();
    //         if ($out7Items->isEmpty()) {
    //             return response()->json(['success' => false, 'message' => 'Records not found']);
    //         }

    //         foreach ($out7Items as $out7) {
    //             $item_id = $out7->item_id;
    //             $qty_request = $out7->qty_request;
    ...
    //         $firstOut7 = StrOut7::where('doc_no', $doc_no)->first();
    //         if ($firstOut7) {
    //             $firstOut7->status = 1; // Example status update
    //             $firstOut7->save();
    //         }
...
//             StrOut7::where('doc_no', $doc_no)->update([
//                 'status' => 1
//             ]);
...
//     }
    ...
    {
        $doc_no = $request->input('doc_no');
        $strOut7 = StrOut7::where('doc_no', $doc_no)->first();
        if ($strOut7) {
            $strOut7->status = 2;
            $strOut7->save();
    ...
    //             $item_id = $out7->item_id;
    //             $qty_out = $out7->qty_out; // 🔥 pakai qty_out
    ...
    //         // Update status semua data str_out7
    //         StrOut7::where('doc_no', $doc_no)->update([
    //             'status' => 2
    //         ]);
    ...
    public function saveChecked(Request $request)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $itemId) {
                ...
                $item = StrOut7::find($itemId); // Ganti dengan model yang sesuai
                if ($item) {
                    // Contoh update field
            ...
            if ($updatedDocNo) {
                $itemToUpdate = StrOut7::where('doc_no', $updatedDocNo)->first();
                if ($itemToUpdate) {
