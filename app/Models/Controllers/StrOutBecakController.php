...
class StrOutBecakController extends Controller
{
    public function index()
    {
        ...
        return view('storeroom.becak', compact('title', 'departements', 'str_uoms', 'str_barangs', 'users'));
    }
    ...
    {
        $query = DB::table('str_out_becaks as a')
    ...
    public function update(Request $request)
    {
        if($request->button == "update"){
            ...
            for ($i=1; $i < $jml+1; $i++) { 
                ...
                StrOutBecak::where('id', $request->idline[$i])->where('id', $request->idket[$i])->update($data);
            }
    ...
    {
        $cek = StrOutBecak::select(DB::raw('COUNT(doc_no) as jml'))->whereMonth('created_at', date('m'))->groupBy('doc_no')->count();
        if($cek > 0){
            $array = array();
            $rows = StrOutBecak::select('doc_no')->whereMonth('created_at', date('m'))->groupBy('doc_no')->get();
...
{
    // Fungsi untuk membersihkan format angka dengan titik ribuan
    function cleanNumber($number) {
        return floatval(str_replace('.', '', $number));
    }

   // Ambil data barang berdasarkan item_id
...
// Bersihkan harga dari titik ribuan lalu konversi ke float
$cleanPrice = str_replace('.', '', $barang->price);
$price = is_numeric($cleanPrice) ? floatval($cleanPrice) : 0;

// Pastikan qty juga valid
$qtyRequest = is_numeric($request->qty_request) ? floatval($request->qty_request) : 0;

// Hitung total harga item
$priceItem = $qtyRequest * $price;
    ...
    public function update2(Request $request)
    {
        ...
        $query = StrOutBecak::where('id', $request->id)->update($data);
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsOutBecakExport($startDate, $endDate), 'Penggunaan Barang Becak.xlsx');
    }

    public function destroyline(Request $request)
    {
        $query = StrOutBecak::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = StrOutBecak::where('id', $request->id)->count();
        if($cek > 0){
            $row = StrOutBecak::where('id', $request->id)->first();
            return response()->json([
                'success'        => true,
                'id'             => $row->id,
                'item_id'        => $row->item_id,
    ...
    {
        $query = StrOutBecak::where('doc_no', $request->doc_no)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = StrOutBecak::where('doc_no', $request->doc_no)->count();
        if($cek > 0){
            $data['sts'] = 1;
            $query = StrOutBecak::where('doc_no', $request->doc_no)->update($data);
            if($query){
                return response()->json([
                    'success'   => true,
                    'msg'       => 'Submit success.'
    ...
    {
        $row = StrOutBecak::select('date_plan','line_id')
    ...
    public function approve(Request $request)
    {
        ...
        try {
            ...
            $out17Items = StrOutBecak::where('doc_no', $doc_no)->get();
            if ($out17Items->isEmpty()) {
                return response()->json(['success' => false, 'message' => 'Records not found']);
            }

            foreach ($out17Items as $out17) {
                $item_id = $out17->item_id;
                $qty_request = $out17->qty_request;
            ...
            $firstOut17 = StrOutBecak::where('doc_no', $doc_no)->first();
            if ($firstOut17) {
                $firstOut17->status = 1; // Example status update
                $firstOut17->save();
            }
    ...
    {
        $doc_no = $request->input('doc_no');
        $StrOutBecak = StrOutBecak::where('doc_no', $doc_no)->first();
        if ($StrOutBecak) {
            $StrOutBecak->status = 2;
            $StrOutBecak->save();
    ...
    public function saveChecked(Request $request)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $itemId) {
                ...
                $item = StrOutBecak::find($itemId); // Ganti dengan model yang sesuai
                if ($item) {
                    // Contoh update field
            ...
            if ($updatedDocNo) {
                $itemToUpdate = StrOutBecak::where('doc_no', $updatedDocNo)->first();
                if ($itemToUpdate) {
