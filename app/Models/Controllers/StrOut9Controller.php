...
use App\Models\StrOut9;
use App\Models\StrCategory;
use App\Models\StrUom;
use App\Models\User;
use Illuminate\Http\Request;
use App\Exports\ItemsOut9Export;
...
class StrOut9Controller extends Controller
{
    public function index()
    {
        ...
        return view('storeroom.out9', compact('title', 'departements', 'str_uoms', 'str_barangs', 'users'));
    }

    public function list()
    {
        $query = DB::table('str_out9s as a')
    ...
    public function update(Request $request)
    {
        if($request->button == "update"){
            ...
            for ($i=1; $i < $jml+1; $i++) { 
                ...
                StrOut9::where('id', $request->idline[$i])->where('id', $request->idket[$i])->update($data);
            }
   ...
   public function getdoc()
{
    ...
    $lastDoc = StrOut9::where('doc_no', 'like', "%/$yearMonth/%")
        ->orderBy('created_at', 'desc')
        ->value('doc_no');
    ...
    public function update2(Request $request)
    {
        ...
        $query = StrOut9::where('id', $request->id)->update($data);
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsOut9Export($startDate, $endDate), 'Penggunaan Barang PSW Manual.xlsx');
    }

    public function destroyline(Request $request)
    {
        $query = StrOut9::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = StrOut9::where('id', $request->id)->count();
        if($cek > 0){
            $row = StrOut9::where('id', $request->id)->first();
            return response()->json([
                'success'        => true,
                'id'             => $row->id,
                'item_id'        => $row->item_id,
    ...
    {
        $query = StrOut9::where('doc_no', $request->doc_no)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = StrOut9::where('doc_no', $request->doc_no)->count();
        if($cek > 0){
            $data['sts'] = 1;
            $query = StrOut9::where('doc_no', $request->doc_no)->update($data);
            if($query){
                return response()->json([
                    'success'   => true,
                    'msg'       => 'Submit success.'
    ...
    {
        $row = StrOut9::select('date_plan','line_id')
    ...
    public function approve(Request $request)
    {
        ...
        try {
            ...
            $out9Items = StrOut9::where('doc_no', $doc_no)->get();
            ...
            $firstOut9 = StrOut9::where('doc_no', $doc_no)->first();
            if ($firstOut9) {
    ...
    {
        $doc_no = $request->input('doc_no');
        $strOut9 = StrOut9::where('doc_no', $doc_no)->first();
        if ($strOut9) {
            $strOut9->status = 2;
            $strOut9->save();
    ...
    public function saveChecked(Request $request)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $itemId) {
                ...
                $item = StrOut9::find($itemId); // Ganti dengan model yang sesuai
                if ($item) {
                    // Contoh update field
            ...
            if ($updatedDocNo) {
                $itemToUpdate = StrOut9::where('doc_no', $updatedDocNo)->first();
                if ($itemToUpdate) {
