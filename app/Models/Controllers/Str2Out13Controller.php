...
use App\Models\Str2Out13;
use App\Models\StrCategory;
use App\Models\StrUom;
use App\Models\User;
use App\Exports\ItemsOut13ExportStr2;
...
class Str2Out13Controller extends Controller
{
    public function index()
    {
        ...
        return view('storeroom2.out13', compact('title', 'departements', 'str_uoms', 'str_barangs', 'users'));
    }

    public function list()
    {
        $query = DB::table('str2_out13s as a')
    ...
    public function update(Request $request)
    {
        if($request->button == "update"){
            ...
            for ($i=1; $i < $jml+1; $i++) { 
                ...
                Str2Out13::where('id', $request->idline[$i])->where('id', $request->idket[$i])->update($data);
            }
    ...
    {
        $cek = Str2Out13::select(DB::raw('COUNT(doc_no) as jml'))->whereMonth('created_at', date('m'))->groupBy('doc_no')->count();
        if($cek > 0){
            $array = array();
            $rows = Str2Out13::select('doc_no')->whereMonth('created_at', date('m'))->groupBy('doc_no')->get();
    ...
    public function update2(Request $request)
    {
        ...
        $query = Str2Out13::where('id', $request->id)->update($data);
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsOut13ExportStr2($startDate, $endDate), 'Penggunaan Barang  PSW & RSW KS ASI-2.xlsx');
    }

    public function destroyline(Request $request)
    {
        $query = Str2Out13::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = Str2Out13::where('id', $request->id)->count();
        if($cek > 0){
            $row = Str2Out13::where('id', $request->id)->first();
            return response()->json([
                'success'        => true,
                'id'             => $row->id,
                'item_id'        => $row->item_id,
    ...
    {
        $query = Str2Out13::where('doc_no', $request->doc_no)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = Str2Out13::where('doc_no', $request->doc_no)->count();
        if($cek > 0){
            $data['sts'] = 1;
            $query = Str2Out13::where('doc_no', $request->doc_no)->update($data);
            if($query){
                return response()->json([
                    'success'   => true,
                    'msg'       => 'Submit success.'
    ...
    {
        $row = Str2Out13::select('date_plan','line_id')
    ...
    public function approve(Request $request)
    {
        ...
        try {
            ...
            $out13Items = Str2Out13::where('doc_no', $doc_no)->get();
            if ($out13Items->isEmpty()) {
                return response()->json(['success' => false, 'message' => 'Records not found']);
            }

            foreach ($out13Items as $out13) {
                $item_id = $out13->item_id;
                $qty_request = $out13->qty_request;
            ...
            $firstOut13 = Str2Out13::where('doc_no', $doc_no)->first();
            if ($firstOut13) {
                $firstOut13->status = 1; // Example status update
                $firstOut13->save();
            }
    ...
    {
        $doc_no = $request->input('doc_no');
        $strOut13 = Str2Out13::where('doc_no', $doc_no)->first();
        if ($strOut13) {
            $strOut13->status = 2;
            $strOut13->save();
    ...
    public function saveChecked(Request $request)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $itemId) {
                ...
                $item = Str2Out13::find($itemId); // Ganti dengan model yang sesuai
                if ($item) {
                    // Contoh update field
            ...
            if ($updatedDocNo) {
                $itemToUpdate = Str2Out13::where('doc_no', $updatedDocNo)->first();
                if ($itemToUpdate) {
