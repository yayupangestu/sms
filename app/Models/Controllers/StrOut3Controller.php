...
USE App\Exports\ItemsOut3SummaryExport;
...
class StrOut3Controller extends Controller
{
    public function index()
    {
        ...
        return view('storeroom.out3', compact('title', 'departements', 'str_uoms', 'str_barangs', 'users'));
    }
    ...
    //         ->join('str_categories as b', 'b.id', '=', 'a.category', 'left')
    //         ->join('str_out3s as c', 'c.item_id', '=', 'a.id', 'left') // Join dengan tabel transaksi
    ...
    public function update(Request $request)
    {
        if($request->button == "update"){
            ...
            for ($i=1; $i < $jml+1; $i++) { 
                ...
                StrOut3::where('id', $request->idline[$i])->where('id', $request->idket[$i])->update($data);
            }
    ...
    public function getdoc()
{
    ...
    $lastDoc = StrOut3::where('doc_no', 'like', "%/$yearMonth/%")
        ->orderBy('created_at', 'desc')
        ->value('doc_no');
    ...
    public function update2(Request $request)
    {
        ...
        $query = StrOut3::where('id', $request->id)->update($data);
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ItemsOut3Export($startDate, $endDate), 'Penggunaan Barang B3.xlsx');
    }
      ...
      public function exportSummary(Request $request)
{
    ...
    return Excel::download(new ItemsOut3SummaryExport($start, $end), 'Summary Item Line B3.xlsx');
}

    public function destroyline(Request $request)
    {
        $query = StrOut3::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = StrOut3::where('id', $request->id)->count();
        if($cek > 0){
            $row = StrOut3::where('id', $request->id)->first();
            return response()->json([
                'success'        => true,
                'id'             => $row->id,
                'item_id'        => $row->item_id,
    ...
    {
        $query = StrOut3::where('doc_no', $request->doc_no)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
    ...
    {
        $cek = StrOut3::where('doc_no', $request->doc_no)->count();
        if($cek > 0){
            $data['sts'] = 1;
            $query = StrOut3::where('doc_no', $request->doc_no)->update($data);
            if($query){
                return response()->json([
                    'success'   => true,
                    'msg'       => 'Submit success.'
    ...
    {
        $row = StrOut3::select('date_plan','line_id')
    ...
    public function approve(Request $request)
    {
        ...
        try {
            ...
            foreach ($out3Items as $out3) {
                $item_id = $out3->item_id;
                $qty_request = $out3->qty_request;
            ...
            $firstOut3 = StrOut3::where('doc_no', $doc_no)->first();
            if ($firstOut3) {
                $firstOut3->status = 1; // Example status update
                $firstOut3->save();
            }
...
private function updateStock($modelClass, $item_id, $qty_request)
{
    ...
    {
        $doc_no = $request->input('doc_no');
        $strOut3 = StrOut3::where('doc_no', $doc_no)->first();
        if ($strOut3) {
            $strOut3->status = 2;
            $strOut3->save();
    ...
    //         foreach ($out2Items as $out2) {

    //             $item_id = $out2->item_id;
    //             $qty_out = $out2->qty_out; // 🔥 pakai qty_out
    ...
    //         // Update status semua data str_out2
    //         StrOut2::where('doc_no', $doc_no)->update([
    //             'status' => 2
    //         ]);
    ...
    public function saveChecked(Request $request)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $itemId) {
                ...
                $item = StrOut3::find($itemId); // Ganti dengan model yang sesuai
                if ($item) {
                    // Contoh update field
            ...
            if ($updatedDocNo) {
                $itemToUpdate = StrOut3::where('doc_no', $updatedDocNo)->first();
                if ($itemToUpdate) {
