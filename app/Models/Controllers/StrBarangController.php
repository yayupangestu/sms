...
class StrBarangController extends Controller
{
    public function index()
    {
        $title = 'Data Barang';
        $str_categories = StrCategory::all();
        return view('storeroom.barang', compact('title','str_categories'));
    }
    

    public function list()
    {
        $query = DB::table('str_barangs as a')
    ...
    {
        $cek = StrBarang::where('id', $request->id)->count();
        if($cek > 0){
            $row = StrBarang::where('id', $request->id)->first();
            return response()->json([
                'success'       => true,
                'id'            => $row->id,
                'name'          => $row->name,
    ...
    public function update(Request $request)
    {
        ...
        $query = StrBarang::where('id', $request->id)->update($data);
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Update success.'
    ...
    {
        $query = StrBarang::where('id', $request->id)->delete();
        if($query){
            return response()->json([
                'success'   => true,
                'msg'       => 'Delete success.'
