...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h1 class="m-0"> E-Surat Permintaan Barang LINE B1 & B2</h1>
                </div>
                <div class="col-sm-6">
                </div>
            </div>
    ...
    <section class="content">
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <form action="{{ route('strout4.update') }}" method="POST">
                            @csrf
                            <div class="CardHead">
                                <div class="card-header">
                                    <h3 class="card-title">
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
            var getActualUrl = '{{ route('strout4.getActual') }}'; // Menghasilkan URL route
        ...
        function getDoc() {
            ...
            $.ajax({
                type: 'GET',
                url: "{{route('strout4.getdoc')}}",
                success: function (result) {
                    $("#id").val('');
                    $("#doc_no").val("SPB/ASI/STAMPING/LINEB/" + year + month + "/" + result.jml);
                    $("#date_plan").val('');
                    $('#line_id').val('');
                    $('#item_id').val('');
                    $("#qty_return").val('');
        ...
        document.getElementById("saveChecked").addEventListener("click", function () {
            ...
            if (checkedItems.length > 0) {
                $.ajax({
                    type: 'POST',
                    url: "{{ route('strout4.saveChecked') }}",  // Sesuaikan URL
                    data: {
                        items: checkedItems,
        ...
        function delete_draft() {
            $.ajax({
                type: 'POST',
                url: "{{route('strout4.delete_draft')}}",
                data: {
                    doc_no: doc_no.value,
                    _token: '{{csrf_token()}}'
                },
        ...
        $(document).on("click", ".export-option", function () {
            ...
            if (type === "history") {
                window.location.href =
                    "{{ route('strout4.export') }}?start_date=" + startDate + "&end_date=" + endDate;
                return;
            }
            ...
            if (type === "data") {
                window.location.href =
                    "{{ route('strout4.export.summary') }}?start_date=" + startDate + "&end_date=" + endDate;
                return;
            }
        });
