...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h1 class="m-0"> E-Surat Permintaan Barang LINE A1 & A2</h1>
                </div>
                <div class="col-sm-6">
                </div>
            </div>
    ...
    <section class="content">
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <form action="{{ route('strout5.update') }}" method="POST">
                            @csrf
                            <div class="CardHead">
                                <div class="card-header">
                                    <h3 class="card-title">
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
            var getActualUrl = '{{ route('strout5.getActual') }}'; // Menghasilkan URL route
        ...
        function getDoc() {
            ...
            $.ajax({
                type: 'GET',
                url: "{{route('strout5.getdoc')}}",
                success: function (result) {
                    $("#id").val('');
                    $("#doc_no").val("SPB/ASI/STAMPING/LINEA/" + year + month + "/" + result.jml);
                    $("#date_plan").val('');
                    $('#line_id').val('');
                    $('#item_id').val('');
                    $("#qty_return").val('');
        ...
        function list() {
            ...
            $(document).on("click", ".btn-details", function () {
                ...
                $.ajax({
                    type: 'GET',
                    url: "{{ route('strout5.listdetail3') }}", // Route yang mengarah ke listdetail3
                    data: {
                        doc_no: doc_no
                    },
                    success: function (result) {
        ...
        document.getElementById("saveChecked").addEventListener("click", function () {
            ...
            if (checkedItems.length > 0) {
                $.ajax({
                    type: 'POST',
                    url: "{{ route('strout5.saveChecked') }}",  // Sesuaikan URL
                    data: {
                        items: checkedItems,
        ...
        function delete_draft() {
            $.ajax({
                type: 'POST',
                url: "{{route('strout5.delete_draft')}}",
                data: {
                    doc_no: doc_no.value,
                    _token: '{{csrf_token()}}'
                },
        ...
        $(document).on("click", ".export-option", function () {
            ...
            if (type === "history") {
                window.location.href =
                    "{{ route('strout5.export') }}?start_date=" + startDate + "&end_date=" + endDate;
                return;
            }
            ...
            if (type === "data") {
                window.location.href =
                    "{{ route('strout5.export.summary') }}?start_date=" + startDate + "&end_date=" + endDate;
                return;
            }
        });
