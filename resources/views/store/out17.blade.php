...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h1 class="m-0">E-Surat Permintaan Barang LINE PAINTING</h1>
                </div>
                <div class="col-sm-6">
                </div>
            </div>
    ...
    <section class="content">
                    ...
                    <div class="card" id="CardHead">
                        <div class="card-header">
                            <h3 class="card-title">
                                ...
                                E-Surat Permintaan Barang PAINTING(SPB)
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <form action="{{ route('strout17.update') }}" method="POST">
                            @csrf
                            <div class="CardHead">
                                <div class="card-header">
                                    <h3 class="card-title">
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
            var getActualUrl = '{{ route('strout17.getActual') }}'; // Menghasilkan URL route
        ...
        function getDoc() {
            ...
            $.ajax({
                type: 'GET',
                url: "{{route('strout17.getdoc')}}",
                success: function (result) {
                    $("#id").val('');
                    $("#doc_no").val("SPB/ASI/PRODUKSI2/PAINTING/" + year + month + "/" + result.jml);
                    $("#date_plan").val('');
                    $('#line_id').val('');
                    $('#item_id').val('');
                    $("#qty_return").val('');
        ...
        function list() {
            ...
            $(document).on("click", ".btn-details", function () {
                ...
                $.ajax({
                    type: 'GET',
                    url: "{{ route('strout17.listdetail3') }}", // Route yang mengarah ke listdetail3
                    data: {
                        doc_no: doc_no
                    },
                    success: function (result) {
        ...
        document.getElementById("saveChecked").addEventListener("click", function () {
            ...
            if (checkedItems.length > 0) {
                $.ajax({
                    type: 'POST',
                    url: "{{ route('strout17.saveChecked') }}",  // Sesuaikan URL
                    data: {
                        items: checkedItems,
        ...
        $(document).on("click", "#btn_search", function () {
            ...
            $.ajax({
                type: 'GET',
                url: "{{route('strout17.listdetail2')}}",
                data: {
                    doc_no: doc_no,
                },
                success: function (result) {
        ...
        function delete_draft() {
            $.ajax({
                type: 'POST',
                url: "{{route('strout17.delete_draft')}}",
                data: {
                    doc_no: doc_no.value,
                    _token: '{{csrf_token()}}'
                },
        ...
        $(document).on("click", "#btn_export", function () {
            ...
            if (startDate && endDate) {
                window.location.href = "{{ route('strout17.export') }}?start_date=" + startDate + "&end_date=" + endDate;
            } else {
                Toast.fire({
                    icon: 'warning',
        ...
        $(document).on("click", "#btn_delete_line", function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{route('strout17.destroyline')}}",
