...
<div class="content-header">
  <div class="container-fluid">
    <div class="row">
      <div class="col-sm-6">
        <h1 class="m-0">E-Surat Permintaan Barang LINE B3</h1>
      </div>
      <div class="col-sm-6">
      </div>
    </div>
  ...
  <div class="container-fluid">
   ...
   <div class="container-fluid">
    <div class="row">
        <div class="col-12">
            <div class="card">
                <form action="{{ route('strout3.update') }}" method="POST">
                    @csrf
                    <div class="CardHead">
                        <div class="card-header">
                            <h3 class="card-title">
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $(document).ready(function() {
                var getActualUrl = '{{ route('strout3.getActual') }}'; // Menghasilkan URL route
            ...
            function getDoc(){
                ...
                $.ajax({
                    type: 'GET',
                    url: "{{route('strout3.getdoc')}}",
                    success: function(result) {
                        $("#id").val('');
                        $("#doc_no").val("SPB/ASI/STAMPING/LINEB3/"+year+month+"/"+result.jml);
                        $("#date_plan").val('');
                        $('#line_id').val('');
                        $('#item_id').val('');
                        $("#qty_return").val('');
          ...
          // Script untuk menangani klik tombol detail dan memuat modal
            $(document).on("click", ".btn-details", function () {
                var doc_no = $(this).data('doc_no'); // Ambil doc_no dari tombol yang diklik
                 ...
                 $.ajax({
                    type: 'GET',
                    url: "{{ route('strout3.listdetail3') }}", // Route yang mengarah ke listdetail3
                        data: {
                            doc_no: doc_no
                        },
                        success: function(result) {
                ...
                document.getElementById("saveChecked").addEventListener("click", function() {
                    ...
                    if (checkedItems.length > 0) {
                        $.ajax({
                            type: 'POST',
                            url: "{{ route('strout3.saveChecked') }}",  // Sesuaikan URL
                            data: {
                                items: checkedItems,
            ...
            function delete_draft(){
                    $.ajax({
                        type: 'POST',
                        url: "{{route('strout3.delete_draft')}}",
                        data: {
                            doc_no: doc_no.value,
                            _token: '{{csrf_token()}}'
                            },
...
$(document).on("click", ".export-option", function () {
    ...
    if (type === "history") {
        window.location.href =
            "{{ route('strout3.export') }}?start_date=" + startDate + "&end_date=" + endDate;
        return;
    }
    ...
    if (type === "data") {
        window.location.href =
            "{{ route('strout3.export.summary') }}?start_date=" + startDate + "&end_date=" + endDate;
        return;
    }
});
