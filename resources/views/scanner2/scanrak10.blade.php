...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Scan QR Code Nut RAK 2</h1>
                </div>
            </div>
        </div>
    </div>
        ...
        if (firstCode === secondCode) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('scanraksepuluh.storeScanOutNut') }}",
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                    ...
                    if (firstCode === "90174-10038") {
                        $.ajax({
                            url: "{{ route('scanraksepuluh.activateRelay') }}",
                            method: 'GET',
                            success: function(response) {
                                Swal.fire({
                                ...
                                setTimeout(function() {
                                    $.ajax({
                                        url: "{{ route('scanraksepuluh.deactivateRelay') }}",
                                        method: 'GET',
                                        success: function() {
                            ...
                            error: function(error) {
                                Swal.fire({
                                    ...
                                    $.ajax({
                                        url: "{{ route('scanraksepuluh.activateRelayDua') }}",
                                        method: 'GET',
                                        success: function (response) {
                                            Swal.fire({
                                    ...
                                    $.ajax({
                                        url: "{{ route('scanraksepuluh.activateRelayTiga') }}",
                                        method: 'GET',
                                        success: function (response) {
                                            Swal.fire({
