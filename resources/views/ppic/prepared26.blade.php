...
.qr-reader-container {
    position: relative;
    width: 100px;   /* kecilkan dari 300px */
    height: 100px;  /* atur tinggi sesuai proporsi */
    margin: auto;
    overflow: hidden;
...
.focus-box {
    ...
    z-index: 10;
    pointer-events: none; /* agar tidak menghalangi klik di area kamera */
}
...
<div class="content-header">
  <div class="container-fluid">
    <div class="row">
      <div class="col-sm-6">
        <h1 class="m-0">List Prepare Deliver TMMIN</h1>
      </div>
      <div class="col-sm-6">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                  <h3 class="card-title">List List Prepare Deliver TMMIN</h3>
...
function loadCycleCounts(docDn) {
    ...
    $('#cycleNumber option').each(function () {
        var val = $(this).val();
        if (val) {
            var data = response[val] || { total: 0, total_gt0: 0, total_qty2: 0 };
            $(this).text(
                'Cycle ' + val + ' (' + data.total + '/' + data.total_gt0 + '/' + data.total_qty2 + ')'
            );
        }
    });
},
...
function startScanner() {
    ...
    qrScanner.start(
        ...
        qrCodeMessage => {
            ...
            if (scanStep === 1) {
    // kalau sudah ada value di #scanResult, jangan overwrite lagi
    if ($('#scanResult').val().trim() !== '') {
        return; // skip scan ulang
    }

    let substringToCheck = qrCodeMessage.substr(3, 10); // manifest
    let partNoExtract = qrCodeMessage.substr(qrCodeMessage.indexOf(' ') + 1, 17).trim(); // part_no hasil scan pertama

    // AJAX cek sts ke server
    $.ajax({
        url: "{{ route('check.sts') }}",
        type: "GET",
        data: {
            manifest: substringToCheck,
            part_no: partNoExtract
        },
    ...
    // kalau sudah ada value di #scanResult2, jangan overwrite lagi
    if ($('#scanResult2').val().trim() !== '') {
        return; // skip scan ulang
    }
...
$('#CekScan').on('click', function() {
    ...
    let indexSpace = scan1.indexOf(' ');
    if (indexSpace === -1) {
        Swal.fire({
            icon: 'error',
            title: 'Format Scan Pertama Salah',
            text: 'Tidak ada spasi pada hasil scan pertama.'
        });
        return;
    }

    let extractedScan1 = scan1.substr(indexSpace + 1, 17).trim();
    let extractedScan2 = scan2.substr(0, 12).replace(/\./g, '').trim();
    let manifestExtract = scan1.substr(3, 10);
    ...
    if (extractedScan1 === extractedScan2) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // Jalankan AJAX update
                $.ajax({
                    url: "{{ route('listrekapd26.update') }}",
                    method: 'POST',
                    data: {
        ...
        Swal.fire({
            ...
            html:
              `<p>Part No CS: <b>${extractedScan1}</b></p>` +
              `<p>Part No ASI: <b>${extractedScan2}</b></p>`
        });
    }
});
