...
<body>
    ...
    style="
      ...
      position: relative;
      display: flex;
      justify-content: center;   /* center untuk judul */
      align-items: center;
    "
  >
    <!-- Jam di pojok kiri -->
    <span id="datetime"
      style="
        position: absolute;
        left: 20px;
        font-size: 70px;   /* sedikit lebih kecil biar pas */
      "
    ></span>
  
    <!-- Judul KAP di tengah -->
    <span id="kapTitle">KAP-2</span>
  </div>
...
//     const pages = [
//       { title: 'KAP-2', visibleCycles: [1,2,3,4] },
//       { title: 'KAP-2', visibleCycles: [] }
//     ];
...
// setInterval(checkRelayAuto, 1000);
    ...
    //     try {
    //         const response = await fetch("{{ route('press.data.adm2') }}");
    ...
    //                         } else if (cycle >= 7 && cycle <= 9) {  // sekarang 7,8,9
    ...
    async function updatePressData() {
        try {
            ...
            // Group data by job_no → biar qty_act bisa dialokasikan
            const groupedByJob = {};
            pressData.forEach(item => {
    ...
    //     try {
    //         const response = await fetch("{{ route('subassy.data.adm2') }}");
    ...
    try {
        const response = await fetch("{{ route('nut.data.adm2') }}");
        ...
        Object.keys(groupedByJob).forEach(jobNo => {
            ...
            jobItems.forEach(data => {
                ...
                // 🔹 Tentukan ikon status (alokasi qty_act)
                let iconHtml;
                if (strength > 1.0) {
                    iconHtml = '<i class="fas fa-check-circle" style="color: #28a745; font-weight:bold; margin-left: 5px;"></i>';
                } else if (remainingAct >= qtyOrder) {
                ...
                const td = document.querySelector(`#nut-row td[data-cycle='${data.cycle}'] .wrap-columns`);
                if (td) td.appendChild(div);
            });
        });
