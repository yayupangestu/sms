...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h1 class="m-0">LIST DELIVERY ADM PLANT 4</h1>
                </div>
                <div class="col-sm-6">
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">LIST DELIVERY ADM PLANT 4</h3>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group row">
                        <label class="col-sm-2 col-form-label font-weight-bold">Kanban ADM:</label>
                        <div class="col-sm-9">
...
@push('scripts')
    ...
    $('#example1').DataTable({
        ...
        pageLength: 10,
        ajax: {
            url: "{{ route('listrekapdadmp4.list') }}"
        },
        columns: [
            {
                data: null,
            ...
            {
                data: 'del_date_group', // ✅ gabungan del_date
                name: 'del_date_group',
                className: 'text-center'
            },
            {
    ...
    render: function (data, type, row) {
        return `
            <button class="btn btn-sm btn-primary open-modal"
                    data-created-at="${data}"
                    data-cycle="${row.cycle ?? ''}"
                    data-docdn="${row.manifest ?? ''}">
        ...
        language: {
            processing: `<img src="{{ asset('dist/img/Hourglass.gif') }}" alt="Loading..." width="40"> Loading...`
        }
    });
}
...
$(document).on('click', '.open-modal', function(e) {
    ...
    var createdAtValue = $(this).data('created-at');
    var cycleField = $(this).data('cycle');
    var docDn = $(this).data('docdn');

    // Simpan tanggal di modal agar bisa digunakan nanti
    $('#editModal').data('created-at', createdAtValue);
    ...
    // Cek apakah cycle sudah dipilih
    if (!cycleField || cycleField === "") {
        // Kosongkan tabel/detail dulu
        $('#detailTable tbody').html(`
            <tr>
                <td colspan="6" class="text-center text-muted">
                    🔸 Silakan pilih cycle terlebih dahulu
                </td>
            </tr>
        `);
        return; // stop, jangan lanjut ambil data
    }

    // Jika cycle sudah dipilih → baru ambil data
    listdetail(createdAtValue, cycleField);
});
        ...
        function loadCycleCounts(docDn) {
            $.ajax({
                url: "{{ route('get.qty.rekapadmp4') }}", // route Laravel untuk ambil jumlah per cycle
                type: "GET",
                data: {
                    docdn: docDn
                },
    ...
    var createdAtValue = $('#editModal').data('created-at'); // 🔥 ambil lagi tanggal dari modal

    // Panggil ulang DataTable dengan filter lengkap
    listdetail(createdAtValue, selectedCycle);
});
        ...
        function listdetail(createdAtValue, cycleField) {
            $('#editModalTable').DataTable({
                ...
                ajax: {
                    url: "{{ route('listrekapdadmp4.listdetail') }}",
                    data: {
    ...
    qrCodeMessage => {
        ...
        if (scanStep === 1) {
            ...
            let jobNoExtract = qrCodeMessage.substr(substringToCheck.length, 7).trim();

            console.log("Manifest:", substringToCheck);
            console.log("Job No:", jobNoExtract);

            $.ajax({
                url: "{{ route('check.stsadmp4') }}",
                type: "GET",
                data: { manifest: substringToCheck, job_no: jobNoExtract },
                success: function(res) {
                    if (res.sts === 1) {
                        showAlert('⚠️ Kanban sudah Scan');
                        playErrorSound();
                        return;
                    }
        ...
        // === STEP 2 ===
            ...
            // Ambil uniqNo dari posisi setelah titik ke-4
            let parts = qrCodeMessage.split('.');
            if (parts.length < 5) {
                showAlert('⚠️ Format QR Code tidak valid!');
                playErrorSound();
                return;
            }

            let uniqNoToCheck = parts[4];
            console.log("UniqNo ke-2:", uniqNoToCheck);

            // Cek status_prepare (label welding)
            $.ajax({
                url: "{{ route('check.label.weldingp4') }}",
                type: "GET",
                data: { no_label_welding: uniqNoToCheck },
                success: function(res) {
                    if (res.exists && res.message === 'Label sudah digunakan') {
                        showAlert('⚠️ Label sudah digunakan!');
                        playErrorSound();
                        return;
                    }

                    // Cek data uniqNo di tabel_transit_pc_stores
                    $.ajax({
                        url: "{{ route('check.pcstorep4') }}",
                        type: "GET",
                        data: { uniqNo: uniqNoToCheck },
                        success: function(res2) {
                            if (!res2.exists) {
                                showAlert('⚠️ Belum di-scan di Pc-Store!');
                                playErrorSound();
                                $('#scanResult2').val('');
                                $('#checkMark2').html('');
                                return;
                            ...
                            // ✅ Jika Pc-Store ditemukan → simpan hasil scan
                            $('#scanAlert').hide();
                            $('#scanResult2').val(qrCodeMessage);
                            $('#checkMark2').html('✅');
                            playSuccessSound();
                            console.log('✅ Data Pc-Store ditemukan:', res2.data);
                        },
                        error: function() {
                            showAlert('⚠️ Gagal cek data Pc-Store!');
                            playErrorSound();
                        }
                    });
                },
                error: function() {
                    showAlert('⚠️ Gagal cek label welding!');
                    playErrorSound();
                }
            });
        }
    ...
    errorMessage => {
        // 🚫 Sembunyikan error decode bawaan
    ...
    console.error("Gagal nyalakan kamera:", err);
        ...
        $('#CekScan').on('click', function() {
            ...
            let substringToCheck = (firstAIndex !== -1) ?
                ...
                scan1;
            let extractedScan1 = scan1.substr(substringToCheck.length, 7).trim();
            ...
            if (extractedScan1 === extractedScan2) {
                Swal.fire({
                    ...
                    html: `<p>QR CS: <b>${extractedScan1}</b></p>` +
                        ...
                        `<p>NO DN extract: <b>${manifestExtract}</b></p>`,
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('listrekapdadmp4.update') }}",
                            method: 'POST',
                            data: {
