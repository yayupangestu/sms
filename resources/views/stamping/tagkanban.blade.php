...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h1 class="m-0">Tag Kanban LINE B3</h1>
                </div>
                <div class="col-sm-6">
...
$(document).on('click', '#selectAll', function () {
    ...
    $.ajax({
        url: "{{ route('kanbanstmp.generateMultipleQrCodes') }}",
        method: 'POST',
        xhrFields: {
...
//// Batas ALam lain
    ...
    $(document).ready(function () {
    $.get("{{ route('kanbanstmp.getPartNos') }}", function (data) {
        if (data.length > 0) {
    ...
    if (partNo) {
        $.get("{{ route('kanbanstmp.data2') }}", { part_no: partNo }, function (data) {
            if (data.length > 0) {
...
$('#uniqNo1').on('change', function () {
    ...
    $('#qtyForm').on('submit', function(e) {
        ...
        $.post("{{ route('kanbanstmp.totalQtyKm') }}", $(this).serialize(), function(response) {
            if (response.success) {
                Swal.fire({
                    icon: 'success',
                    title: 'Berhasil',
    ...
    $('#qtyForm').on('submit', function(e) {
        ...
        $('#sumQtyForm').on('submit', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('kanbanstmp.totalQtyKm') }}", // URL dengan nama route
                        method: 'POST',
                        data: {
                            uniqNo1: uniqNo1,
                            uniqNo2: uniqNo2,
        ...
        $(document).ready(function() {
    $.get("{{ route('kanbanstmp.data') }}", function(data) {
        if (data.length > 0) {
        ...
        $('#part_no_rm').val(partNo); // Set nilai input part_no_rm
            ...
            $('#kode_material').change(function() {
                ...
                if (selectedValue) {
                    ...
                    $.post("{{ route('kanbanstmp.getQtyStamping') }}", {
    ...
    var table = $('#example1').DataTable({
        ...
        function listdetail() {
            var table = $('#example2').DataTable({
                ...
                ajax: {
                    url: "{{ route('kanbanstmp.listdetail') }}",
                    data: {
        ...
        $(document).on('click', '#btn_pdf', function(e) {
            ...
            var printUrl = "{{ route('kanbanstmp.cetak', ':id') }}".replace(':id', id);
        ...
        function delete_draft() {
            $.ajax({
                type: 'POST',
                url: "{{ route('kanbanstmp.delete_draft') }}",
                data: {
                    date_plan: date_plan.value,
                    _token: '{{ csrf_token() }}'
                },
        ...
        $(document).on("click", "#btn_delete_line", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('kanbanstmp.destroyline') }}",
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
