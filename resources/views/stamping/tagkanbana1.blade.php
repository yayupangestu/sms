...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h1 class="m-0">Tag Kanban LINE A1</h1>
                </div>
                <div class="col-sm-6">
            ...
            <div class="col-12">
                <div class="card shadow-sm rounded">
                    <div class="modal-header" style="background-color: #2e3b4e; color:#ffffff">
                        <h3 class="card-title m-0">List Data Kanban A1</h3>
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div  class="modal-header" style="background-color: rgb(0, 11, 68); color:white">
                    <h4 class="modal-title" id="title1">Create Your Kanban LINE A1</h4>
                    <h4 class="modal-title" id="title2">Edit Your Kanban LINE A1</h4>
...
$(document).on('click', '#selectAll', function () {
    ...
    $.ajax({
        url: "{{ route('kanbanstmpa1.generateMultipleQrCodes') }}",
        method: 'POST',
        xhrFields: {
...
//// Batas ALam lain
    ...
    $(document).ready(function () {
    $.get("{{ route('kanbanstmpa1.getPartNos') }}", function (data) {
        if (data.length > 0) {
    ...
    if (partNo) {
        $.get("{{ route('kanbanstmpa1.data2') }}", { part_no: partNo }, function (data) {
            if (data.length > 0) {
...
$('#uniqNo1').on('change', function () {
    ...
    $('#qtyForm').on('submit', function(e) {
        ...
        $.post("{{ route('kanbanstmpa1.totalQtyKm') }}", $(this).serialize(), function(response) {
            if (response.success) {
                Swal.fire({
                    icon: 'success',
                    title: 'Berhasil',
    ...
    $('#qtyForm').on('submit', function(e) {
        ...
        $('#sumQtyForm').on('submit', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('kanbanstmpa1.totalQtyKm') }}", // URL dengan nama route
                        method: 'POST',
                        data: {
                            uniqNo1: uniqNo1,
                            uniqNo2: uniqNo2,
        ...
        $(document).ready(function() {
    $.get("{{ route('kanbanstmpa1.data') }}", function(data) {
        if (data.length > 0) {
        ...
        $('#part_no_rm').val(partNo); // Set nilai input part_no_rm
            ...
            $('#kode_material').change(function() {
                ...
                if (selectedValue) {
                    ...
                    $.post("{{ route('kanbanstmpa1.getQtyStamping') }}", {
    ...
    var table = $('#example1').DataTable({
        ...
        function listdetail() {
            var table = $('#example2').DataTable({
                ...
                ajax: {
                    url: "{{ route('kanbanstmpa1.listdetail') }}",
                    data: {
        ...
        $(document).on('click', '#btn_pdf', function(e) {
            ...
            var printUrl = "{{ route('kanbanstmpa1.cetak', ':id') }}".replace(':id', id);
        ...
        function delete_draft() {
            $.ajax({
                type: 'POST',
                url: "{{ route('kanbanstmpa1.delete_draft') }}",
                data: {
                    date_plan: date_plan.value,
                    _token: '{{ csrf_token() }}'
                },
        ...
        $(document).on("click", "#btn_delete_line", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('kanbanstmpa1.destroyline') }}",
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
        ...
        $(document).on("click", "#btn_delete", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('kanbanstmpa1.destroy') }}",
                        data: {
                            date_plan: date_plan,
                            _token: '{{ csrf_token() }}'
                        },
