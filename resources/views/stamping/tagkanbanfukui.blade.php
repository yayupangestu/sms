...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h1 class="m-0">Tag Kanban LINE FUKUI</h1>
                </div>
                <div class="col-sm-6">
            ...
            <div class="col-12">
                <div class="card shadow-sm rounded">
                    <div class="modal-header" style="background-color: #2e3b4e; color:#ffffff">
                        <h3 class="card-title m-0">List Data Kanban FUKUI</h3>
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header" style="background-color: rgb(0, 11, 68); color:white">
                    <h4 class="modal-title" id="title1">Create Your Kanban fukui</h4>
                    <h4 class="modal-title" id="title2">Edit Your Kanban fukui</h4>
...
@push('scripts')
    ...
    <script>
        ...
        $(document).on('click', '#btn_generate_all', function(e) {
            ...
            $.ajax({
                url: "{{ route('kanbanstmpfukui.generateMultipleQrCodes2') }}",
                method: 'POST',
                xhrFields: {
        ...
        $(document).ready(function() {
            $.get("{{ route('kanbanstmpfukui.getPartNos') }}", function(data) {
                if (data.length > 0) {
        ...
        $(document).ready(function() {
            ...
            $('#km_part').on('change', function() {
                ...
                if (partNo) {
                    $.get("{{ route('kanbanstmpfukui.data2') }}", {
                        part_no: partNo
                    }, function(data) {
                        if (data.length > 0) {
            ...
            $('#qtyForm').on('submit', function(e) {
                ...
                $.post("{{ route('kanbanstmpfukui.totalQtyKm') }}", $(this).serialize(), function(
                    response) {
                    if (response.success) {
                        Swal.fire({
                            icon: 'success',
        ...
        $('#qtyForm').on('submit', function(e) {
            ...
            $.post("{{ route('kanbanstmpfukui.totalQtyKm') }}", $(this).serialize(), function(response) {
                if (response.success) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Berhasil',
        ...
        $('#sumQtyForm').on('submit', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('kanbanstmpfukui.totalQtyKm') }}", // URL dengan nama route
                        method: 'POST',
                        data: {
                            uniqNo1: uniqNo1,
                            uniqNo2: uniqNo2,
        ...
        $(document).ready(function() {
            $.get("{{ route('kanbanstmpfukui.data') }}", function(data) {
                if (data.length > 0) {
        ...
        $('#kode_material').change(function() {
            ...
            if (selectedValue) {
                ...
                $.post("{{ route('kanbanstmpfukui.getQtyStamping') }}", {
        ...
        //     $.ajax({
        //         type: 'GET',
        //         url: "{{ route('kanbanstmpfukui.getdoc') }}",
        ...
        function list() {
            var table = $('#example1').DataTable({
                ...
                pageLength: 10,
                ajax: {
                    url: "{{ route('kanbanstmpfukui.list') }}"
                },
                order: [
                    [1, 'desc']
                ],
        ...
        function listdetail() {
            var table = $('#example2').DataTable({
                ...
                ajax: {
                    url: "{{ route('kanbanstmpfukui.listdetail') }}",
                    data: {
        ...
        $(document).on('click', '#btn_pdf', function(e) {
            ...
            var printUrl = "{{ route('kanbanstmpfukui.cetak', ':id') }}".replace(':id', id);
        ...
        $(document).on("click", "#btn_edit", function() {
            ...
            $.ajax({
                type: 'GET',
                url: "{{ route('kanbanstmpfukui.edit') }}",
                data: {
                    date_plan: date_plan,
                    _token: '{{ csrf_token() }}'
                },
        ...
        function updt_submit() {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('kanbanstmpfukui.submit') }}",
                data: {
                    date_plan: date_plan.value,
                    _token: '{{ csrf_token() }}'
                },
        ...
        function delete_draft() {
            $.ajax({
                type: 'POST',
                url: "{{ route('kanbanstmpfukui.delete_draft') }}",
                data: {
                    date_plan: date_plan.value,
                    _token: '{{ csrf_token() }}'
                },
        ...
        $(document).on("click", ".Save", function() {
            ...
            if (validasi()) {
                $.ajax({
                    type: 'POST',
                    url: "{{ route('kanbanstmpfukui.store') }}",
                    data: {
                        date_plan: date_plan.value,
                        line_id: line_id.value,
        ...
        $(document).on("click", ".Update", function() {
            if (validasi()) {
                $.ajax({
                    type: 'POST',
                    url: "{{ route('kanbanstmpfukui.update') }}",
                    data: {
                        id: id.value,
                        date_plan: date_plan.value,
                        line_id: line_id.value,
        ...
        $(document).on("click", "#btn_delete_line", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('kanbanstmpfukui.destroyline') }}",
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
        ...
        $(document).on("click", "#btn_delete", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('kanbanstmpfukui.destroy') }}",
                        data: {
                            date_plan: date_plan,
                            _token: '{{ csrf_token() }}'
                        },
