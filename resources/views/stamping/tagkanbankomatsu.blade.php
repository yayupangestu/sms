...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h1 class="m-0">Tag Kanban LINE KOMATSU</h1>
                </div>
                <div class="col-sm-6">
    ...
    <section class="content">
        ...
        <div class="row">
            <a href="dashboardplanningblank" style="margin-left: 20px;">
            ...
            <div class="col-12">
                <div class="card shadow-sm rounded">
                    <div class="modal-header" style="background-color: #2e3b4e; color:#ffffff">
                        <h3 class="card-title m-0">List Data Kanban KOMATSU</h3>
    ...
    <div class="modal fade" id="myModal2">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header" style="background-color: rgb(0, 11, 68); color:white">
                    <h4 class="modal-title" id="title1">Create Your Kanban KOMATSU</h4>
                    <h4 class="modal-title" id="title2">Edit Your Kanban KOMATSU</h4>
                ...
                <div class="modal-body">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-3">
                            <input style="width: 100%" type="hidden" id="doc_no" class="form-control form-control-sm"
                                required>
                        </div>
                            ...
                            <select style="width: 100%;" id="item_id" class="form-control select2" required>
                                ...
                                @foreach ($data_fg_stampings as $part)
                                    <option value="{{ $part->id }}"
                                        data-job_no="{{ $part->job_no }}" data-part_no="{{ $part->part_no2 }}"
                                        data-model="{{ $part->model_id }}" data-qty_plan2="{{ $part->qty_plan2 }}"
                        ...
                        <div class="col-sm-1">
                            <input type="text" input="numeric" id="qty_ok" class="form-control form-control-sm"
                                required>
                        </div>
                        ...
                        <div class="col-sm-2">
                            <select style="width: 100%;" id="tujuan" class="form-control select2" required>
                                ...
                                <option value="LINE B3">LINE B2</option>
                                <option value="LINE C1">LINE C1</option>
                                <option value="LINE C2">LINE C2</option>
                                {{-- <option value="REPIAR">REPAIR</option> --}}
                            </select>
                        </div>

                         <!-- {{-- <label class="col-sm-1 col-form-label">Uniq:</label> --}} -->
                        <div class="col-sm-1">
                            <input type="hidden" id="uniqNo" class="form-control form-control-sm" required>
                        </div>

                        <label class="col-sm-1 col-form-label">Coil:</label>
                        <div class="col-sm-1">
                            <input type="checkbox" id="cekcoil" class="form-control form-control-sm" required>
                        </div>
                        ...
                        <div class="col-sm-7">
                            ...
                            <button type="button" class="btn btn-info btn-sm SumQty" data-bs-toggle="modal"
                                data-bs-target="#sumQtyModal">Sum Qty</button>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#myModal2').on('shown.bs.modal', function() {
                $('#line_id').val('1').trigger(
                'change'); // set value dan trigger select2 update jika pakai select2
            });
        });
        ...
        $(document).on('click', '#selectAll', function() {
            ...
            $('.dataCheckbox').each(function() {
                ...
                const stsText = row.find('span.badge').text().trim();

                // Cek jika status bukan "Sudah Scan"
                if (stsText !== 'Sudah Scan') {
                    $(this).prop('checked', isChecked);
                } else {
                    $(this).prop('checked', false); // pastikan tidak ter-check
                }
            });
        });
        ...
        $(document).on('click', '#btn_generate_all', function(e) {
            ...
            $.ajax({
                url: "{{ route('kanbanstmpkomatsu.generateMultipleQrCodes2') }}",
                method: 'POST',
                xhrFields: {
                ...
                success: function(response) {
                    ...
                    link.download = 'label stamping' + new Date().toISOString().slice(0, 10) + '.pdf';
        ...
        $(document).ready(function() {
            $.get("{{ route('kanbanstmpkomatsu.getPartNos') }}", function(data) {
                if (data.length > 0) {
                    ...
                    data.forEach(item => {
                        ...
                        if (item.status_proses == 2) {
                            ...
                            options +=
                                `<option value="${item.part_no}" style="background-color: #d4edda; border: 1px solid #28a745;">${displayText}</option>`;
                        }
                    });
        ...
        $(document).ready(function() {
            ...
            $('#km_part').on('change', function() {
                ...
                if (partNo) {
                    $.get("{{ route('kanbanstmpkomatsu.data2') }}", {
                        part_no: partNo
                    }, function(data) {
                        if (data.length > 0) {
                            ...
                            data.forEach(item => {
                                options +=
                                    `<option value="${item.uniqNo}">${item.part_no} - ${item.uniqNo} - (${item.qty_stamping})</option>`;
                            });
            ...
            $('#qtyForm').on('submit', function(e) {
                ...
                $.post("{{ route('kanbanstmpkomatsu.totalQtyKm') }}", $(this).serialize(), function(
                    response) {
                    if (response.success) {
                        Swal.fire({
                            icon: 'success',
        ...
        $('#qtyForm').on('submit', function(e) {
            ...
            $.post("{{ route('kanbanstmpkomatsu.totalQtyKm') }}", $(this).serialize(), function(response) {
                if (response.success) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Berhasil',
        ...
        $('#sumQtyForm').on('submit', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('kanbanstmpkomatsu.totalQtyKm') }}", // URL dengan nama route
                        method: 'POST',
                        data: {
                            uniqNo1: uniqNo1,
                            uniqNo2: uniqNo2,
                        ...
                        success: function(response) {
                            ...
                            if (response.success) {
                                ...
                                $('#totalQtyResult').show(); // Menddampilkan hasil di modal
        ...
        $(document).ready(function() {
            $.get("{{ route('kanbanstmpkomatsu.data') }}", function(data) {
                if (data.length > 0) {
        ...
        $('#kode_material').change(function() {
            ...
            if (selectedValue) {
                ...
                $.post("{{ route('kanbanstmpkomatsu.getQtyStamping') }}", {
...
// Validasi qty_ok > qty_plan2
document.getElementById('qty_ok').addEventListener('input', function () {

    // Jika checkbox COIL dicentang → validasi dimatikan
    if (document.getElementById('cekcoil').checked) {
...
// Checkbox COIL → hanya membuat input fleksibel
document.getElementById('cekcoil').addEventListener('change', function () {
    const qtyOkInput = document.getElementById('qty_ok');

    // Selalu biarkan bebas diisi
    qtyOkInput.removeAttribute('readonly');
});
        ...
        //     $.ajax({
        //         type: 'GET',
        //         url: "{{ route('kanbanstmpkomatsu.getdoc') }}",
        ...
        function list() {
            var table = $('#example1').DataTable({
                ...
                pageLength: 10,
                ajax: {
                    url: "{{ route('kanbanstmpkomatsu.list') }}"
                },
                order: [
                    [1, 'desc']
                ],
        ...
        function listdetail() {
            var table = $('#example2').DataTable({
                ...
                ajax: {
                    url: "{{ route('kanbanstmpkomatsu.listdetail') }}",
                    data: {
        ...
        $(document).on('click', '#btn_pdf', function(e) {
            ...
            var printUrl = "{{ route('kanbanstmpkomatsu.cetak', ':id') }}".replace(':id', id);
        ...
        $(document).on("click", "#btn_edit", function() {
            ...
            $.ajax({
                type: 'GET',
                url: "{{ route('kanbanstmpkomatsu.edit') }}",
                data: {
                    date_plan: date_plan,
                    _token: '{{ csrf_token() }}'
                },
        ...
        function updt_submit() {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('kanbanstmpkomatsu.submit') }}",
                data: {
                    date_plan: date_plan.value,
                    _token: '{{ csrf_token() }}'
                },
        ...
        function delete_draft() {
            $.ajax({
                type: 'POST',
                url: "{{ route('kanbanstmpkomatsu.delete_draft') }}",
                data: {
                    date_plan: date_plan.value,
                    _token: '{{ csrf_token() }}'
                },
        ...
        $(document).on("click", ".Save", function() {
            ...
            if (validasi()) {
                $.ajax({
                    type: 'POST',
                    url: "{{ route('kanbanstmpkomatsu.store') }}",
                    data: {
                        date_plan: date_plan.value,
                        line_id: line_id.value,
        ...
        $(document).on("click", ".Update", function() {
            if (validasi()) {
                $.ajax({
                    type: 'POST',
                    url: "{{ route('kanbanstmpkomatsu.update') }}",
                    data: {
                        id: id.value,
                        date_plan: date_plan.value,
                        line_id: line_id.value,
        ...
        function validasi() {
            $("#alert").show();
            if (date_plan.value != '' && line_id.value != '' && item_id.value != '' && qty_ok.value != '' && qty_ng.value !=
                '' && shift.value != '' && tujuan.value != '' && kode_material.value != ''
            ) {
                return true;
            } else {
                $("#alert").html(
        ...
        $(document).on("click", "#btn_delete_line", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('kanbanstmpkomatsu.destroyline') }}",
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
        ...
        $(document).on("click", "#btn_delete", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('kanbanstmpkomatsu.destroy') }}",
                        data: {
                            date_plan: date_plan,
                            _token: '{{ csrf_token() }}'
                        },
