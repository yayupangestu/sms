...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h1 class="m-0">Tag Kanban LINE C2</h1>
                </div>
                <div class="col-sm-6">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-1">
                            <input type="number" id="qty_ok" class="form-control form-control-sm" required>
                        </div>

                        <label class="col-sm-1 col-form-label">Qty NG :</label>
                        <div class="col-sm-1">
                            <input type="number" id="qty_ng" class="form-control form-control-sm" required>
                        </div>
...
/// BATAS ALAM LAIN
    ...
    $('#myModal2').on('show.bs.modal', function () {
        ...
        $(document).ready(function () {
    $.get("{{ route('kanbanstmpc2.getPartNos') }}", function (data) {
        if (data.length > 0) {
    ...
    $('#km_part').on('change', function() {
        ...
        if (partNo) {
            $.get("{{ route('kanbanstmpc2.data2') }}", { part_no: partNo }, function(data) {
    if (data.length > 0) {
        let options = '<option value="">Pilih Kode Material</option>';
        data.forEach(item => {
            options += `<option value="${item.uniqNo}">${item.part_no} - (${item.qty_stamping}) - (${item.uniqNo})</option>`;
        });
    ...
    $('#qtyForm').on('submit', function(e) {
        ...
        $.post("{{ route('kanbanstmpc2.totalQtyKm') }}", $(this).serialize(), function(response) {
            if (response.success) {
                Swal.fire({
                    icon: 'success',
                    title: 'Berhasil',
    ...
    $('#qtyForm').on('submit', function(e) {
        ...
        $('#sumQtyForm').on('submit', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('kanbanstmpc2.totalQtyKm') }}", // URL dengan nama route
                        method: 'POST',
                        data: {
                            uniqNo1: uniqNo1,
                            uniqNo2: uniqNo2,
        ...
        $(document).ready(function() {
    $.get("{{ route('kanbanstmpc2.data') }}", function(data) {
        if (data.length > 0) {
        ...
        $('#part_no_rm').val(partNo); // Set nilai input part_no_rm
            ...
            $('#kode_material').change(function() {
                ...
                if (selectedValue) {
                    ...
                    $.post("{{ route('kanbanstmpc2.getQtyStamping') }}", {
    ...
    var table = $('#example1').DataTable({
        ...
        function listdetail() {
            var table = $('#example2').DataTable({
                ...
                ajax: {
                    url: "{{ route('kanbanstmpc2.listdetail') }}",
                    data: {
        ...
        $(document).on('click', '#btn_pdf', function(e) {
            ...
            var printUrl = "{{ route('kanbanstmpc2.cetak', ':id') }}".replace(':id', id);
        ...
        function delete_draft() {
            $.ajax({
                type: 'POST',
                url: "{{ route('kanbanstmpc2.delete_draft') }}",
                data: {
                    date_plan: date_plan.value,
                    _token: '{{ csrf_token() }}'
                },
        ...
        $(document).on("click", "#btn_delete_line", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('kanbanstmpc2.destroyline') }}",
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
        ...
        $(document).on("click", "#btn_delete", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('kanbanstmpc2.destroy') }}",
                        data: {
                            date_plan: date_plan,
                            _token: '{{ csrf_token() }}'
                        },
