...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h1 class="m-0">Tag Kanban LINE TRANSFERS</h1>
                </div>
                <div class="col-sm-6">
    ...
    <section class="content">
        ...
        <div class="row">
            <a href="dashboardplanningatransfers" style="margin-left: 20px;">
            ...
            <div class="col-12">
                <div class="card shadow-sm rounded">
                    <div class="modal-header" style="background-color: #2e3b4e; color:#ffffff">
                        <h3 class="card-title m-0">List Data Kanban TRANSFERS</h3>
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div  class="modal-header" style="background-color: rgb(0, 11, 68); color:white">
                    <h4 class="modal-title" id="title1">Create Your Kanban LINE TRANSFERS</h4>
                    <h4 class="modal-title" id="title2">Edit Your Kanban LINE TRANSFERS</h4>
...
$(document).on('click', '#selectAll', function () {
    ...
    $.ajax({
        url: "{{ route('kanbanstmptransfers.generateMultipleQrCodes') }}",
        method: 'POST',
        xhrFields: {
...
//// Batas ALam lain
    ...
    $(document).ready(function () {
    $.get("{{ route('kanbanstmptransfers.getPartNos') }}", function (data) {
        if (data.length > 0) {
    ...
    if (partNo) {
        $.get("{{ route('kanbanstmptransfers.data2') }}", { part_no: partNo }, function (data) {
            if (data.length > 0) {
...
$('#uniqNo1').on('change', function () {
    ...
    $('#qtyForm').on('submit', function(e) {
        ...
        $.post("{{ route('kanbanstmptransfers.totalQtyKm') }}", $(this).serialize(), function(response) {
            if (response.success) {
                Swal.fire({
                    icon: 'success',
                    title: 'Berhasil',
    ...
    $('#qtyForm').on('submit', function(e) {
        ...
        $('#sumQtyForm').on('submit', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('kanbanstmptransfers.totalQtyKm') }}", // URL dengan nama route
                        method: 'POST',
                        data: {
                            uniqNo1: uniqNo1,
                            uniqNo2: uniqNo2,
        ...
        $(document).ready(function() {
    $.get("{{ route('kanbanstmptransfers.data') }}", function(data) {
        if (data.length > 0) {
    ...
    $('#kode_material').change(function() {
        ...
        $('#part_no_rm').val(partNo); // Set nilai input part_no_rm
            ...
            $('#kode_material').change(function() {
                ...
                if (selectedValue) {
                    ...
                    $.post("{{ route('kanbanstmptransfers.getQtyStamping') }}", {
        ...
        //     $.ajax({
        //         type: 'GET',
        //         url: "{{ route('kanbanstmptransfers.getdoc') }}",
    ...
    var table = $('#example1').DataTable({
        ...
        pageLength: 10,
        ajax: {
            url: "{{ route('kanbanstmptransfers.list') }}"
        },
        order: [[1, 'desc']],
        ...
        function listdetail() {
            var table = $('#example2').DataTable({
                ...
                ajax: {
                    url: "{{ route('kanbanstmptransfers.listdetail') }}",
                    data: {
        ...
        $(document).on('click', '#btn_pdf', function(e) {
            ...
            var printUrl = "{{ route('kanbanstmptransfers.cetak', ':id') }}".replace(':id', id);
        ...
        $(document).on("click", "#btn_edit", function() {
            ...
            $.ajax({
                type: 'GET',
                url: "{{ route('kanbanstmptransfers.edit') }}",
                data: {
                    date_plan: date_plan,
                    _token: '{{ csrf_token() }}'
                },
        ...
        function updt_submit() {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('kanbanstmptransfers.submit') }}",
                data: {
                    date_plan: date_plan.value,
                    _token: '{{ csrf_token() }}'
                },
        ...
        function delete_draft() {
            $.ajax({
                type: 'POST',
                url: "{{ route('kanbanstmptransfers.delete_draft') }}",
                data: {
                    date_plan: date_plan.value,
                    _token: '{{ csrf_token() }}'
                },
        ...
        $(document).on("click", ".Save", function() {
            ...
            if (validasi()) {
                $.ajax({
                    type: 'POST',
                    url: "{{ route('kanbanstmptransfers.store') }}",
                    data: {
                        date_plan: date_plan.value,
                        line_id: line_id.value,
        ...
        $(document).on("click", ".Update", function() {
            if (validasi()) {
                $.ajax({
                    type: 'POST',
                    url: "{{ route('kanbanstmptransfers.update') }}",
                    data: {
                        id: id.value,
                        date_plan: date_plan.value,
                        line_id: line_id.value,
        ...
        $(document).on("click", "#btn_delete_line", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('kanbanstmptransfers.destroyline') }}",
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
        ...
        $(document).on("click", "#btn_delete", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('kanbanstmptransfers.destroy') }}",
                        data: {
                            date_plan: date_plan,
                            _token: '{{ csrf_token() }}'
                        },
