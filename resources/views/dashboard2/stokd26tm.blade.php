...
<head>
    ...
    <title>Stock Strength Dashboard (Hour) TMMIN</title>
...
<body>
    <div class="title">STOCK STRENGTH CHART TMMIN</div>
    <div class="page">
        <div class="sheet">
            <div class="header">
    ...
    <script>
        ...
        async function loadPcStoreData() {
            try {
                const res = await fetch("{{ route('pcstore.dataTmmin') }}");
...
async function renderTransit() {
    try {
        const res = await fetch("{{ route('scan_out_weldings.dataTmmin') }}");
        const rows = await res.json();
