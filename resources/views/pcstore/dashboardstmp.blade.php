...
function animateRowMovement(tableId, newData) {
    ...
    setTimeout(() => {
        ...
        newData.forEach((item, index) => {
            ...
            // ✅ Kolom actual_production blink merah jika description_sts = 1
            const actualHtml = `
                <td style="font-size:25px">
                    <span class="${item.description_sts == 1 ? 'blink-red' : ''}">
                        ${item.actual_production != null ? item.actual_production : 0}
                    </span>
                </td>
            `;
            ...
            row.innerHTML = `
                ...
                <td style="font-size:25px">${item.part_no2}</td>
                <td style="font-size:25px">${item.model_id || '-'}</td>
                <td style="font-size:25px">${item.qty_plan}</td>
                ${actualHtml}
                <td style="font-size:20px">${getStatusLabel(item.status_proses, item.qty_out_material, item.qty_plan, item.status_proses2, item.sts_wip)}</td>
                <td style="font-size:25px">${item.time_startProses || '-'}</td>
                <td style="font-size:25px">${item.time_endProses || '-'}</td>
                <td style="font-size:25px">${item.description || '-'}</td>
            `;

            // ✨ Efek highlight untuk row yang bergerak
...
function updateTableData(data) {
    ...
    animateRowMovement("table3000",    data.planningData3000      || []);
}
