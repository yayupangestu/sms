...
@section('content')
    <style>
        ...
        .summary-item {
            flex: 1 1 300px;
            background: linear-gradient(135deg, #f9f9f9, #86868621);
            padding: 20px;
            border-radius: 12px;
...
.modal-dialog.modal-xl-custom {
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h1 class="m-0">Dashboard All Stok Pc Store</h1>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="container-fluid">
            <div class="orders-summary">
                <div class="summary-item">
                    <h3>INFORMATION ALL PART PC STORE</h3>
                    @php
                    ...
                    <div class="resume-section mt-3">
                        ...
                        <div class="resume-box stock-safe" style="padding: 10px; border: 1px solid #ffffff; border-radius: 6px; text-align: center;">
                            <h4 style="color: rgb(0, 0, 0)">READY STOK</h4>
                            @php
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="orders-chart text-left"
                             style="padding:20px; background:#ffffff; border-radius:10px;">
                            <h5 style="color:#000000;">Stock Status Chart 2</h5>
            ...
            <table class="orders-data-table table table-hover table-striped align-middle">
                <thead class="table-dark text-center">
                    <tr>
                        ...
                        <th scope="col" style="min-width: 300px;" class="text-start">Part Name</th>
                        <th scope="col" style="min-width: 120px;" class="text-center">Part No</th>
...
@push('scripts')
                ...
                columns: [
                    ...
                    { data: 'qty_act' },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            badgeHtml = '<span style="background-color:#ea9999; color:#000000; padding:4px 8px; border-radius:4px;">OVER</span>';
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (strengthValue >= 1.0) {
                            ...
                            badgeHtml = '<span class="badge bg-warning">Over</span>';
                        }
                        else {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            const filteredData = data.filter(item => parseFloat(item.strength) > 1.0);
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            const filteredData = data.filter(item => {
                let strengthValue = parseFloat(item.strength);
                return strengthValue >= 0.5 && strengthValue < 1.0;
            });
...
$('.stock-danger').on('click', function() {
    $.ajax({
        ...
        success: function(response) {
            ...
            const filteredData = data.filter(item => parseFloat(item.strength) < 0.5);
        ...
        error: function(xhr, status, error) {
            ...
            let stockChart2 = null;

// Load chart data
async function loadStockChart(customer = "") {
    const response = await fetch("{{ route('stock.chart.data') }}?customer=" + customer);
    ...
    const ctx = document.getElementById('stockChart2');

    // If chart exists -> update only
    if (stockChart2) {
        stockChart2.data.labels = labels;
        stockChart2.data.datasets[0].data = qtyAct;
        stockChart2.data.datasets[1].data = monthlyVolume;
        stockChart2.update();
        return;
    }

    // Create chart first time
    stockChart2 = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: labels,
            datasets: [
        ...
        options: {
            ...
            plugins: {
                ...
                zoom: {
                    ...
                    zoom: {
                        ...
                        pinch: {
                            enabled: true,   // pinch on touch screen
                        },
                        mode: 'x',
                    }
                }
            ...
            scales: {
                ...
                y: { ticks: { color:'#000' }, grid: { color:'#ccc' }, beginAtZero: true }
            }
        }
    });
}
